...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Starting balance monitoring at {timezone.now()}"))
        ...
        try:
            ...
            self.stdout.write(f"Found {len(low_balance_students)} students with balance below {threshold} hours")
            ...
            self.stdout.write(f"Found {len(expiring_packages)} packages expiring within {expiry_days} days")
            ...
            if dry_run:
                ...
                self.stdout.write(self.style.WARNING("DRY RUN COMPLETE - No notifications sent"))
                return
            ...
            # Report results
            self.stdout.write(self.style.SUCCESS("Balance monitoring completed successfully!"))
            ...
            raise CommandError(f"Error during balance monitoring: {e!s}")

        self.stdout.write(self.style.SUCCESS(f"Balance monitoring completed at {timezone.now()}"))
