...
from accounts.models import InvitationStatus, SchoolMembership, SchoolRole, TeacherInvitation
...
class NotificationCountBusinessLogicTest(MessagingTestBase):
    ...
    def test_count_pending_teacher_invitations(self):
        ...
        self.create_teacher_invitation(email="pending@test.com", status=InvitationStatus.PENDING, expires_in_days=7)

        # Create accepted invitation (should NOT be counted)
        self.create_teacher_invitation(email="accepted@test.com", status=InvitationStatus.ACCEPTED, expires_in_days=7)
        ...
        pending_count = TeacherInvitation.objects.filter(
            school=self.school, status=InvitationStatus.PENDING, expires_at__gt=timezone.now()
        ).count()
    ...
    def test_count_new_registrations_requiring_first_login(self):
        ...
        self.create_student_user(email="existing@test.com", name="Existing User", first_login_completed=True)

        # Business rule: count only users who haven't completed first login
        new_registration_count = SchoolMembership.objects.filter(
            school=self.school, user__first_login_completed=False
        ).count()
    ...
    def test_count_incomplete_profiles(self):
        ...
        incomplete_user = self.create_teacher_user(
            email="incomplete@test.com", name="Incomplete User", first_login_completed=True
        )
        ...
        complete_user = self.create_teacher_user(
            email="complete@test.com", name="Complete User", first_login_completed=True
        )
        ...
        incomplete_profile_count = Task.objects.filter(
            user__in=school_users, title__icontains="Complete Your Profile", status="pending", is_system_generated=True
        ).count()
    ...
    def test_count_overdue_tasks(self):
        ...
        overdue_user = self.create_teacher_user(email="overdue@test.com", name="Overdue User")
        Task.objects.create(
            user=overdue_user,
            title="Overdue Task",
            status="pending",
        ...
        future_user = self.create_teacher_user(email="future@test.com", name="Future User")
        Task.objects.create(
            user=future_user,
            title="Future Task",
            status="pending",
        ...
        completed_user = self.create_teacher_user(email="completed@test.com", name="Completed User")
        Task.objects.create(
            user=completed_user,
        ...
        overdue_count = Task.objects.filter(
            user__in=school_users, status="pending", due_date__lt=timezone.now()
        ).count()
    ...
    def test_school_scoped_counting(self):
        ...
        self.create_teacher_invitation(email="our_teacher@test.com", status=InvitationStatus.PENDING, expires_in_days=7)
        ...
        other_school_count = TeacherInvitation.objects.filter(
            school=other_school, status=InvitationStatus.PENDING, expires_at__gt=timezone.now()
        ).count()
