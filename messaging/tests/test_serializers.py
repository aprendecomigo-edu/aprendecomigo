...
class TestNotificationSerializer(BaseAPITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.user = CustomUser.objects.create_user(email="user@example.com", name="Test User")
...
class TestSchoolEmailTemplateSerializer(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.creator = CustomUser.objects.create_user(email="admin@example.com", name="Admin User")
    ...
    def test_template_variable_validation(self):
        ...
        for template_content in valid_templates:
            with self.subTest(content=template_content):
                ...
                serializer = SchoolEmailTemplateSerializer(data=data)
                self.assertTrue(
                    serializer.is_valid(), f"Should accept template: {template_content}. Errors: {serializer.errors}"
                )
    ...
    def test_created_by_auto_assignment(self):
        ...
        serializer = SchoolEmailTemplateSerializer(data=data, context={"request": mock_request})
    ...
    def test_read_only_fields_protection(self):
        ...
        read_only_fields = ["id", "created_by_name", "school_name", "template_variables", "created_at", "updated_at"]
    ...
    def test_custom_css_validation(self):
        ...
        for css in safe_css:
            with self.subTest(css=css[:30]):
                ...
                serializer = SchoolEmailTemplateSerializer(data=data)
                self.assertTrue(
                    serializer.is_valid(), f"Should accept safe CSS: {css[:30]}. Errors: {serializer.errors}"
                )
...
class TestEmailTemplatePreviewSerializer(BaseAPITestCase):
    ...
    def test_template_preview_validation(self):
        ...
        data = {"template_variables": {"teacher_name": "João Silva", "school_name": "Test School"}}

        serializer = EmailTemplatePreviewSerializer(data=data, context={"template": self.template})
        self.assertTrue(serializer.is_valid())
...
class TestEmailCommunicationSerializer(BaseAPITestCase):
    ...
    def setUp(self):
        super().setUp()
        self.school = School.objects.create(name="Test School")
        self.sender = CustomUser.objects.create_user(email="sender@example.com", name="Sender User")
    ...
    def test_delivery_status_tracking(self):
        ...
        status_values = ["queued", "sent", "delivered", "opened", "clicked", "failed", "bounced"]
    ...
    def test_trigger_event_validation(self):
        ...
        for event in valid_events:
            with self.subTest(event=event):
                ...
                self.assertTrue(serializer.is_valid(), f"Should accept event: {event}. Errors: {serializer.errors}")
...
class TestEmailSequenceStepSerializer(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.sequence = EmailSequence.objects.create(
            school=self.school, name="Test Sequence", trigger_event="invitation_sent", is_active=True
        )
    ...
    def test_send_condition_validation(self):
        ...
        valid_conditions = ["always", "no_response", "no_open", "no_click", "conditional"]
...
class TestPurchaseTransactionSerializer(BaseAPITestCase):
    ...
    def test_purchase_transaction_serialization(self):
        ...
        expected_fields = ["id", "transaction_type", "amount", "payment_status", "expires_at", "created_at"]
        for field in expected_fields:
            self.assertIn(field, data)
...
class TestNotificationMarkReadResponseSerializer(BaseAPITestCase):
    ...
    def test_mark_read_response_serialization(self):
        ...
        self.assertTrue(serializer.is_valid())
        self.assertEqual(serializer.validated_data["message"], "Notification marked as read successfully")
...
class TestMessagingSerializersIntegration(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.admin = CustomUser.objects.create_user(email="admin@example.com", name="Admin User")
        self.student = CustomUser.objects.create_user(email="student@example.com", name="Student User")
    ...
    def test_complete_email_workflow_serialization(self):
        ...
        template_serializer = SchoolEmailTemplateSerializer(data=template_data, context={"request": mock_request})
        self.assertTrue(template_serializer.is_valid())

        # Test preview
        preview_data = {"template_variables": {"name": "João Silva", "school_name": "Integration Test School"}}

        if template_serializer.is_valid():
            template = template_serializer.save()
            preview_serializer = EmailTemplatePreviewSerializer(data=preview_data, context={"template": template})
    ...
    def test_security_across_all_serializers(self):
        ...
        self.assertTrue(
            template_serializer.is_valid(), f"Safe template should be valid. Errors: {template_serializer.errors}"
        )
