...
class SNSSMSProvider:
    ...
    async def send_sms_async(self, to: str, message: str, from_number: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            logger.error(f"SNS SMS error to {to}: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'provider': 'sns',
    ...
    def _send_sms_sync(self, to: str, message: str, from_number: Optional[str] = None) -> Dict:
...
class SMSService:
    ...
    async def send_async(self, to: str, message: str, from_number: Optional[str] = None) -> Dict:
        """Send SMS asynchronously."""
        return await self.provider.send_sms_async(to, message, from_number)
    
    def send(self, to: str, message: str, from_number: Optional[str] = None) -> Dict:
        """Send SMS synchronously."""
        return self.provider.send_sms(to, message, from_number)
    
    async def send_bulk_async(self, recipients: List[str], message: str, from_number: Optional[str] = None) -> List[Dict]:
    ...
    def send_bulk(self, recipients: List[str], message: str, from_number: Optional[str] = None) -> List[Dict]:
...
def send_sms(to: str, message: str, from_number: Optional[str] = None) -> Dict:
    """Send a single SMS message."""
    return get_sms_service().send(to, message, from_number)


async def send_sms_async(to: str, message: str, from_number: Optional[str] = None) -> Dict:
    """Send a single SMS message asynchronously."""
    return await get_sms_service().send_async(to, message, from_number)


def send_bulk_sms(recipients: List[str], message: str, from_number: Optional[str] = None) -> List[Dict]:
    """Send SMS to multiple recipients."""
    return get_sms_service().send_bulk(recipients, message, from_number)


async def send_bulk_sms_async(recipients: List[str], message: str, from_number: Optional[str] = None) -> List[Dict]:
    """Send SMS to multiple recipients asynchronously."""
