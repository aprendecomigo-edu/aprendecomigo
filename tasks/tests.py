...
from .serializers import TaskCommentSerializer, TaskCreateSerializer, TaskSerializer, TaskUpdateSerializer
...
class TaskModelTest(BaseTestCase):
    ...
    def test_task_creation(self):
        ...
        task = Task.objects.create(
            title="Test Task", description="Test Description", user=self.user, priority="high", task_type="personal"
        )
    ...
    def test_is_overdue_property(self):
        ...
        task_no_due = Task.objects.create(title="No Due Date", user=self.user)
        self.assertFalse(task_no_due.is_overdue)

        # Completed task is never overdue
        task_completed = Task.objects.create(
            title="Completed Task", user=self.user, due_date=now - timedelta(days=1), status="completed"
        )
        self.assertFalse(task_completed.is_overdue)

        # Past due date and not completed
        task_overdue = Task.objects.create(
            title="Overdue Task", user=self.user, due_date=now - timedelta(days=1), status="pending"
        )
        self.assertTrue(task_overdue.is_overdue)

        # Future due date
        task_future = Task.objects.create(
            title="Future Task", user=self.user, due_date=now + timedelta(days=1), status="pending"
        )
    ...
    def test_days_until_due_property(self):
        ...
        task_tomorrow = Task.objects.create(title="Tomorrow Task", user=self.user, due_date=now + timedelta(hours=25))
        self.assertEqual(task_tomorrow.days_until_due, 1)

        # Task due 25 hours ago (negative number)
        task_yesterday = Task.objects.create(title="Yesterday Task", user=self.user, due_date=now - timedelta(hours=25))
    ...
    def test_is_overdue_boundary_conditions(self):
        ...
        task_due_now = Task.objects.create(title="Due Now Task", user=self.user, due_date=now, status="pending")
        ...
        task_just_past = Task.objects.create(
            title="Just Past Task", user=self.user, due_date=now - timedelta(seconds=1), status="pending"
        )
        ...
        task_just_future = Task.objects.create(
            title="Just Future Task", user=self.user, due_date=now + timedelta(seconds=1), status="pending"
        )
    ...
    def test_days_until_due_calculation_logic(self):
        ...
        with patch("django.utils.timezone.now", return_value=fixed_time):
            # Task due in 25 hours (should be 1 day)
            task_25h = Task.objects.create(title="25H Task", user=self.user, due_date=fixed_time + timedelta(hours=25))
            self.assertEqual(task_25h.days_until_due, 1)

            # Task due in 23 hours (should be 0 days)
            task_23h = Task.objects.create(title="23H Task", user=self.user, due_date=fixed_time + timedelta(hours=23))
            self.assertEqual(task_23h.days_until_due, 0)

            # Task due exactly now
            task_now = Task.objects.create(title="Due Now Task", user=self.user, due_date=fixed_time)
            self.assertEqual(task_now.days_until_due, 0)

            # Task due 1 day ago
            task_past = Task.objects.create(title="Past Task", user=self.user, due_date=fixed_time - timedelta(days=1))
...
class TaskSerializerTest(BaseTestCase):
    ...
    def test_due_date_validation_past_date(self):
        ...
        # Test TaskSerializer
        task_data = {"title": "Test Task", "due_date": past_date}
    ...
    def test_due_date_validation_create_serializer(self):
        ...
        create_data = {"title": "Test Task", "due_date": past_date}
    ...
    def test_due_date_validation_update_serializer(self):
        ...
        update_data = {"title": "Updated Task", "due_date": past_date}
    ...
    def test_due_date_validation_allows_future_date(self):
        ...
        valid_data = {"title": "Future Task", "due_date": future_date}
    ...
    def test_due_date_validation_allows_none(self):
        ...
        valid_data = {"title": "No Due Date Task", "due_date": None}
...
class TaskCustomActionUnitTest(BaseTestCase):
    ...
    def test_summary_action_logic(self):
        ...
        Task.objects.create(title="Overdue", user=self.user, status="pending", due_date=now - timedelta(days=1))
        Task.objects.create(title="Urgent", user=self.user, status="pending", is_urgent=True)
        Task.objects.create(
            title="Due Today",
            user=self.user,
            status="pending",
            due_date=today_start + timedelta(hours=23, minutes=59),  # End of today - guaranteed not to be overdue
        )
    ...
    def test_reopen_action_logic(self):
        ...
        task = Task.objects.create(title="Completed Task", user=self.user, status="completed")
...
class TaskCommentViewSetUnitTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(title="Test Task", user=self.user)
        self.other_user_task = Task.objects.create(title="Other User Task", user=self.other_user)
    ...
    def test_get_queryset_filters_by_task_and_user(self):
        ...
        comment1 = TaskComment.objects.create(task=self.task, user=self.user, comment="Comment on user's task")
        TaskComment.objects.create(
            task=self.other_user_task, user=self.other_user, comment="Comment on other user's task"
        )
...
class TaskViewSetBusinessLogicTest(BaseTestCase):
    ...
    def test_task_comment_serializer_includes_user_name(self):
        ...
        task = Task.objects.create(title="Test Task", user=self.user)
        comment = TaskComment.objects.create(task=task, user=self.user, comment="Test comment")
