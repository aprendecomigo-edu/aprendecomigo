...
class TaskListView(LoginRequiredMixin, View):
    ...
    def _filter_tasks(self, tasks, filter_type):
        ...
        if filter_type == "pending":
            ...
            return tasks.filter(due_date__lt=now, status__in=["pending", "in_progress"])
    ...
    def _get_task_summary(self, user):
        ...
        return {
            ...
            "overdue_tasks": tasks.filter(due_date__lt=now, status__in=["pending", "in_progress"]).count(),
            "urgent_tasks": tasks.filter(is_urgent=True, status__in=["pending", "in_progress"]).count(),
            "tasks_due_today": tasks.filter(
                due_date__gte=today_start, due_date__lt=today_end, status__in=["pending", "in_progress"]
            ).count(),
            "tasks_due_this_week": tasks.filter(
                due_date__gte=today_start, due_date__lt=week_end, status__in=["pending", "in_progress"]
            ).count(),
        }
    ...
    def _handle_create_task(self, request):
        ...
        try:
            ...
            if not title:
                return render(request, "tasks/partials/error_message.html", {"error": "Task title is required"})
            ...
            if due_date:
                try:
                    ...
                    if parsed_due_date < timezone.now():
                        return render(
                            request, "tasks/partials/error_message.html", {"error": "Due date cannot be in the past"}
                        )
                except ValueError:
                    return render(request, "tasks/partials/error_message.html", {"error": "Invalid due date format"})
            ...
            return render(
                request, "tasks/partials/error_message.html", {"error": "Failed to create task. Please try again."}
            )
    ...
    def _handle_complete_task(self, request):
        ...
        try:
            ...
            return render(request, "tasks/partials/error_message.html", {"error": "Failed to complete task"})
    ...
    def _handle_reopen_task(self, request):
        ...
        try:
            ...
            return render(request, "tasks/partials/error_message.html", {"error": "Failed to reopen task"})
    ...
    def _handle_delete_task(self, request):
        ...
        try:
            ...
            return render(request, "tasks/partials/error_message.html", {"error": "Failed to delete task"})
...
class TaskDetailView(LoginRequiredMixin, View):
    ...
    def get(self, request, task_id):
        ...
        if request.headers.get("HX-Request"):
            return render(request, "tasks/partials/task_form.html", {"task": task, "edit_mode": True})
    ...
    def post(self, request, task_id):
        ...
        try:
            ...
            response = render(request, "tasks/partials/success_message.html", {"message": "Task updated successfully!"})
            response["HX-Trigger"] = "taskUpdated"
            return response

        except Exception as e:
            logger.error(f"Error updating task: {e}")
            return render(request, "tasks/partials/error_message.html", {"error": "Failed to update task"})
...
class TaskSummaryView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, "tasks/partials/task_summary.html", {"summary": summary})
