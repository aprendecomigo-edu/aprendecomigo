...
class TaskService:
    ...
    @classmethod
    def create_verification_tasks(cls, user: CustomUser, email: str, phone_number: str | None = None) -> list[Task]:
        """
    ...
    def complete_email_verification_task(cls, user: CustomUser) -> bool:
        ...
        try:
            email_task = Task.objects.filter(
                user=user, title="Verify your email address", status__in=["pending", "in_progress"]
            ).first()
    ...
    def complete_phone_verification_task(cls, user: CustomUser) -> bool:
        ...
        try:
            phone_task = Task.objects.filter(
                user=user, title="Verify your phone number", status__in=["pending", "in_progress"]
            ).first()
    ...
    def get_verification_tasks(cls, user: CustomUser) -> dict:
        ...
        """
        email_task = Task.objects.filter(user=user, title="Verify your email address").first()

        phone_task = Task.objects.filter(user=user, title="Verify your phone number").first()
