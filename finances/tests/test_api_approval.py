...
from accounts.models import ParentChildRelationship, RelationshipType
...
class ApprovalAPITestCase(FinanceBaseTestCase, APITestCase):
    ...
    def setUp(self):
        ...
        self.other_parent = User.objects.create_user(email="other_parent@test.com", name="Other Parent")
...
class FamilyBudgetControlAPITests(ApprovalAPITestCase):
    ...
    def test_create_budget_control_as_parent(self):
        ...
        FamilyBudgetControl.objects.filter(parent_child_relationship=self.parent_child_relationship).delete()
    ...
    def test_list_budget_controls_filtered_by_parent(self):
        ...
        budget_control, created = FamilyBudgetControl.objects.get_or_create(
            ...
            defaults={"monthly_budget_limit": Decimal("200.00"), "auto_approval_threshold": Decimal("20.00")},
        )
        ...
        FamilyBudgetControl.objects.create(
            parent_child_relationship=other_relationship, monthly_budget_limit=Decimal("150.00")
        )
    ...
    def test_budget_limit_validation_logic(self):
        ...
        try:
            budget_control = FamilyBudgetControl.objects.get(parent_child_relationship=self.parent_child_relationship)
    ...
    def test_child_cannot_modify_budget_controls(self):
        ...
        data = {"parent_child_relationship": self.parent_child_relationship.id, "monthly_budget_limit": "100.00"}

        response = self.client.post(url, data, format="json")

        # Should be forbidden, bad request (validation), or return empty list
        self.assertIn(
            response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST]
        )
...
class PurchaseApprovalRequestAPITests(ApprovalAPITestCase):
    ...
    def test_parent_list_pending_approval_requests(self):
        ...
        other_relationship = ParentChildRelationship.objects.create(
            parent=self.other_parent, child=other_child, relationship_type=RelationshipType.PARENT, school=self.school
        )
    ...
    def test_approve_purchase_request(self):
        ...
        data = {"action": "approve", "parent_notes": "Approved for educational purposes"}
    ...
    def test_deny_purchase_request_with_reason(self):
        ...
        data = {"action": "deny", "parent_notes": "Too expensive this month, try again next month"}
...
class StudentPurchaseRequestAPITests(ApprovalAPITestCase):
    ...
    def test_manual_approval_required_for_large_amount(self):
        ...
        self.budget_control.auto_approval_threshold = Decimal(
            "50.00"
        )  # Below the test amount so manual approval needed
        self.budget_control.save()
    ...
    def test_invalid_parent_relationship_validation(self):
        ...
        unrelated_parent = User.objects.create_user(email="unrelated@test.com", name="Unrelated Parent")
...
class ParentApprovalDashboardAPITests(ApprovalAPITestCase):
    ...
    def setUp(self):
        ...
        self.child2 = User.objects.create_user(email="child2@test.com", name="Child Two")

        self.relationship2 = ParentChildRelationship.objects.create(
            parent=self.parent, child=self.child2, relationship_type=RelationshipType.PARENT, school=self.school
        )

        # Create or update budget controls
        FamilyBudgetControl.objects.update_or_create(
            parent_child_relationship=self.parent_child_relationship,
            defaults={"monthly_budget_limit": Decimal("200.00"), "auto_approval_threshold": Decimal("50.00")},
        )
        FamilyBudgetControl.objects.create(
