...
class WaffleSwitchTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.school = School.objects.create(name="Test School", description="School for waffle switch testing")
        ...
        SchoolMembership.objects.create(user=self.user, school=self.school, role="teacher")
    ...
    def test_schedule_urls_return_404_when_switch_off(self):
        ...
        for url in schedule_urls:
            with self.subTest(url=url):
                ...
                self.assertEqual(response.status_code, 404, f"Expected 404 for {url} when schedule_feature is OFF")
    ...
    def test_schedule_urls_accessible_when_switch_on(self):
        ...
        for url in schedule_urls:
            with self.subTest(url=url):
                response = self.client.get(url)
                self.assertNotEqual(
                    response.status_code, 404, f"Expected non-404 status for {url} when schedule_feature is ON"
                )
...
class ChatFeatureSwitchTest(WaffleSwitchTestCase):
    ...
    def test_chat_urls_return_404_when_switch_off(self):
        ...
        for url in chat_urls:
            with self.subTest(url=url):
                response = self.client.get(url)
                self.assertEqual(response.status_code, 404, f"Expected 404 for {url} when chat_feature is OFF")
    ...
    def test_chat_urls_accessible_when_switch_on(self):
        ...
        for url in chat_urls:
            with self.subTest(url=url):
                response = self.client.get(url)
                self.assertNotEqual(
                    response.status_code, 404, f"Expected non-404 status for {url} when chat_feature is ON"
                )
