...
from .models import StudentPayment, TeacherCompensation, PaymentPlan
...
class StudentPaymentService:
    ...
    def is_payment_expired(student_payment):
        ...
        if student_payment.payment_plan.plan_type == "package" and student_payment.payment_plan.expiration_period > 0:
    ...
    def get_active_payment_plans(student, class_type=None):
        ...
        for plan in package_plans:
            if not StudentPaymentService.is_payment_expired(plan) and plan.remaining_hours > 0:
        ...
        if class_type:
            active_plans = [
                ...
                if plan.payment_plan.class_type is None or plan.payment_plan.class_type == class_type
            ]
...
class TeacherCompensationService:
    ...
    def create_compensation_for_period(teacher, start_date, end_date):
        ...
        amount_owed = TeacherCompensationService.calculate_compensation_amount(teacher, class_sessions)
...
class FinancialUtilities:
    ...
    def generate_financial_summary(start_date=None, end_date=None):
        ...
        # Calculate totals
        total_income = student_payments.aggregate(total=Sum('amount_paid'))['total'] or 0
        total_expenses = teacher_compensations.aggregate(total=Sum('amount_owed'))['total'] or 0
        ...
        for plan in payment_plans:
            ...
            plan_stats[plan.name] = {
                ...
                'total': plan_payments.aggregate(total=Sum('amount_paid'))['total'] or 0
            }
