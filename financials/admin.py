...
class PaymentPlanAdmin(admin.ModelAdmin):
    list_display = ('name', 'plan_type', 'rate', 'hours_included', 'expiration_period', 'class_type')
    list_filter = ('plan_type', 'class_type')
    search_fields = ('name', 'description')
    fieldsets = (
        (None, {
            'fields': ('name', 'description', 'plan_type', 'rate')
        }),
        (_('Package Details'), {
...
class StudentPaymentAdmin(admin.ModelAdmin):
    list_display = ('student', 'payment_plan', 'amount_paid', 'payment_date', 'status', 'remaining_hours', 'is_expired')
    ...
    fieldsets = (
        (None, {
            'fields': ('student', 'payment_plan', 'amount_paid', 'payment_date', 'status', 'notes')
        }),
        ...
        (_('Package Plan Details'), {
            'fields': ('hours_purchased', 'hours_used', 'remaining_hours', 'is_expired'),
            'classes': ('collapse',),
            'description': _('Only applicable for package plans')
        }),
    )
    ...
    remaining_hours.short_description = _('Remaining Hours')
...
class TeacherCompensationAdmin(admin.ModelAdmin):
    list_display = ('teacher', 'period_start', 'period_end', 'hours_taught', 'amount_owed', 'amount_paid', 'status', 'is_fully_paid')
    ...
    fieldsets = (
        (None, {
            'fields': ('teacher', 'period_start', 'period_end', 'status')
        }),
        (_('Financial Information'), {
            'fields': ('hours_taught', 'amount_owed', 'amount_paid', 'payment_date', 'is_fully_paid')
        }),
        (_('Class Sessions'), {
            'fields': ('class_sessions',)
        }),
    ...
    is_fully_paid.short_description = _('Fully Paid')
