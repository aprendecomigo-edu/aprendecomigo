...
class BaseViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.message2 = Message.objects.create(
            channel=self.channel1,
            sender=self.user2,
            content="Test message from user2"
        )
...
class ChatViewTest(BaseViewTest):
    ...
    def test_chat_view_create_dm_channel_htmx(self):
        ...
        dm_channel = Channel.objects.filter(is_direct=True, participants=self.user1).filter(participants=new_user).first()
