...
def get_school_users(schools, exclude_user=None):
    ...
    memberships = (
        SchoolMembership.objects.filter(school__in=schools)
        .select_related("user")
...
class ChatUserSearchView(WaffleSwitchMixin, View):
    ...
    def get(self, request):
        ...
        for user_data in school_users:
            ...
            if (
                query_lower in user_data["first_name"].lower()
                or query_lower in user_data["last_name"].lower()
                or query_lower in user_data["email"].lower()
                or query_lower in user_data["username"].lower()
            ):
...
class ChatChannelsView(WaffleSwitchMixin, View):
    ...
    def get(self, request):
        ...
        channels = (
            ...
            .prefetch_related(
                Prefetch(
                    "participants", queryset=User.objects.only("id", "username", "first_name", "last_name", "email")
                ),
        ...
        for channel in channels:
            ...
            last_message = None
            last_message_obj = (
                Message.objects.filter(channel=channel).select_related("sender").order_by("-timestamp").first()
            )
            if last_message_obj:
                last_message = {
    ...
    def post(self, request):
        ...
        try:
            ...
            if is_direct:
                ...
                existing_dm = (
                    Channel.objects.filter(is_direct=True, participants=request.user)
                ...
                channel = Channel.objects.create(name=channel_name, is_direct=False)
...
class ChatMessagesView(WaffleSwitchMixin, View):
    ...
    def get(self, request, channel_id):
        ...
        channel = get_object_or_404(Channel.objects.filter(participants=request.user), id=channel_id)
    ...
    def post(self, request, channel_id):
        ...
        try:
            ...
            # Create message
            message = Message.objects.create(channel=channel, sender=request.user, content=content, file=file_upload)
            ...
            try:
                ...
                # Log WebSocket broadcast failures but don't break the API response
                logger.warning("Failed to broadcast message via WebSocket: %s", str(e))
...
class MessageReactionsView(WaffleSwitchMixin, View):
    ...
    def get(self, request, message_id):
        ...
        message = get_object_or_404(Message.objects.filter(channel__participants=request.user), id=message_id)
    ...
    def post(self, request, message_id):
        ...
        try:
            ...
            reaction, created = Reaction.objects.get_or_create(
                message=message, user=request.user, emoji=emoji, defaults={"emoji": emoji}
            )
...
class ChatView(WaffleSwitchMixin, View):
    ...
    def get(self, request):
        ...
        if selected_channel:
            messages = (
                Message.objects.filter(channel=selected_channel)
                .select_related("sender")
                .prefetch_related("reactions__user")
    ...
    def _load_channel_messages(self, request):
        ...
        return render(
            request,
            "classroom/partials/messages_list.html",
            {
                "messages": messages,
                "channel": channel,
                "user": request.user,
    ...
    def _handle_send_message(self, request):
        ...
        message = Message.objects.create(channel=channel, sender=request.user, content=content)
        ...
        try:
            ...
            # Log HTMX WebSocket broadcast failures but don't break the response
            logger.warning("Failed to broadcast HTMX message via WebSocket: %s", str(e))

        # Return the new message HTML
        return render(
            request,
            "classroom/partials/message_item.html",
            {
                "message": message,
                "user": request.user,
            },
    ...
    def _handle_create_channel(self, request):
        ...
        if existing_dm:
            ...
            channels = (
                Channel.objects.filter(participants=request.user)
                .prefetch_related("participants", "online")
                .order_by("-updated_at")
            )

            return render(
                request,
                "classroom/partials/channels_list.html",
                {
                    "channels": channels,
    ...
    def _search_users(self, request):
        ...
        if not query or len(query) < 2:
            return render(
                request,
                "classroom/partials/user_search_results.html",
                {
                    "search_results": [],
                    "query": query,
                },
...
def chat_school_users(request):
    ...
    return JsonResponse(
        {"users": school_users, "schools": [{"id": school.id, "name": school.name} for school in user_schools]}
    )
