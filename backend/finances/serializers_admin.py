...
class TransactionHistoryQuerySerializer(serializers.Serializer):
    ...
    search = serializers.CharField(
        required=False,
        max_length=255,
        help_text="Search in payment intent ID, charge ID, or student name"
    )
    
    ordering = serializers.CharField(
        required=False,
        help_text="Order by field (e.g., 'created_at', '-amount')"
    )
...
class WebhookStatusQuerySerializer(serializers.Serializer):
    ...
    status = serializers.CharField(
        required=False,
        help_text="Filter by webhook processing status"
    )
    ...
    search = serializers.CharField(
        required=False,
        max_length=255,
        help_text="Search in event ID or event type"
    )
    
    ordering = serializers.CharField(
        required=False,
        help_text="Order by field (e.g., 'created_at', '-retry_count')"
    )
    ...
    date_to = serializers.DateTimeField(
        required=False,
        help_text="Filter events up to this date"
    )
