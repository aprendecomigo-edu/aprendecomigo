...
class FinancialAnalyticsAPIView(APIView):
    ...
    def _get_user_schools(self):
        ...
        user_roles = SchoolRole.objects.filter(
            user=self.request.user,
            role__in=['owner', 'admin']
        )
        return School.objects.filter(
            id__in=user_roles.values_list('school_id', flat=True)
        )
...
def student_spending_analytics(request: Request) -> Response:
    ...
    try:
        ...
        if not hasattr(request.user, 'account_balance'):
            return Response(
                {"error": "Only students can access spending analytics"}, 
        ...
        for transaction_type in TransactionType.choices:
            type_total = transactions.filter(
...
def school_financial_overview(request: Request) -> Response:
    ...
    try:
        ...
        active_students = school_students.filter(
            purchase_transactions__payment_status=TransactionPaymentStatus.COMPLETED
        ).distinct().count()
        ...
        top_students = transactions.values(
        ...
        return Response(
            {"error": "Failed to generate school overview"}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def teacher_compensation_report(request: Request) -> Response:
    ...
    try:
        ...
        hourly_breakdown = sessions.values('date').annotate(
            hours_taught=Count('id'),
            daily_compensation=Count('id') * float(hourly_rate)
        ).order_by('date')
        ...
        return Response(
            {"error": "Failed to generate compensation report"}, 
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
class ExportStudentBalancesAPIView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response(
                {"error": "Failed to export student balances"}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ExportTeacherSessionsAPIView(APIView):
    ...
    def _export_teacher_sessions(self, params):
        ...
        if format_type != 'html':
            return Response(
                {"error": "Only HTML format is supported"}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not all([teacher_id, month, year]):
            return Response(
                {"error": "teacher_id, month, and year parameters are required"}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        response['Content-Disposition'] = f'attachment; filename="teacher_{teacher_id}_sessions_{year}_{month:02d}.html"'
...
<body>
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response(
                {"error": "Failed to export teacher sessions"}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
def list_receipts(request: Request) -> Response:
    ...
    try:
        ...
        # Add download URLs
        for receipt_data, receipt in zip(serializer.data, paginated_receipts):
