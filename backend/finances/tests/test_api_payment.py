...
class PaymentAPITestCase(FinanceBaseTestCase, APITestCase):
    ...
    def setUp(self):
        ...
        self.other_student = User.objects.create_user(
            email="other@example.com", name="Other Student", password="testpass123"
        )
...
class PaymentMethodAPITests(PaymentAPITestCase):
    ...
    def test_add_payment_method_success(self, mock_add_method):
        ...
        data = {"stripe_payment_method_id": "pm_test_new_valid_id_12345678", "is_default": False}
        ...
        mock_add_method.assert_called_once_with(
            student_user=self.student_user, stripe_payment_method_id="pm_test_new_valid_id_12345678", is_default=False
        )
    ...
    def test_add_payment_method_invalid_stripe_id_format(self):
        ...
        for stripe_id in invalid_ids:
            data = {"stripe_payment_method_id": stripe_id, "is_default": False}
            response = self.client.post(url, data)

            self.assertEqual(
                response.status_code, status.HTTP_400_BAD_REQUEST, f"Should reject invalid ID: {stripe_id}"
            )
    ...
    def test_remove_payment_method_success(self, mock_remove_method):
        ...
        mock_remove_method.assert_called_once_with(
            student_user=self.student_user, payment_method_id=self.payment_method.id
        )
...
class PurchaseInitiationAPITests(PaymentAPITestCase):
    ...
    def test_initiate_package_purchase_authenticated(self):
        ...
        data = {
            ...
            "student_info": {"name": self.student_user.name, "email": self.student_user.email},
        }
        ...
        transaction = PurchaseTransaction.objects.get(id=response.data["transaction_id"])
    ...
    def test_initiate_guest_purchase(self):
        ...
        data = {
            "plan_id": self.package_plan.id,
            "student_info": {"name": "Guest Student", "email": "guest@example.com"},
        }
...
@override_settings(STRIPE_SECRET_KEY="sk_test_fake", STRIPE_WEBHOOK_SECRET="whsec_test_fake")
@comprehensive_stripe_mocks_decorator(apply_to_class=True)
class StripeWebhookAPITests(FinanceBaseTestCase, TestCase):
    ...
    def setUp(self):
        ...
        self.student = User.objects.create_user(email="webhook.student@test.com", name="Test Student")
    ...
    def test_payment_intent_succeeded_webhook(self):
        ...
        mock_event = {
            ...
            "data": {
                "object": {
                    ...
                    "currency": "eur",
                    "status": "succeeded",
                    "metadata": {"user_id": str(self.student.id), "transaction_type": "package"},
                }
            },
        }
        ...
        response = self.client.post(
            url, data=payload, content_type="application/json", HTTP_STRIPE_SIGNATURE="test_signature"
        )
    ...
    def test_webhook_idempotent_processing(self):
        ...
        mock_event = {
            "id": "evt_test_123",
            "type": "payment_intent.succeeded",
            "data": {"object": {"id": "pi_test_123456789", "status": "succeeded"}},
        }
        # The comprehensive mocking will handle webhook event construction automatically
...
class PaymentSecurityTests(PaymentAPITestCase):
    ...
    def test_payment_method_data_isolation(self):
        ...
        student_pm = StoredPaymentMethod.objects.create(
            student=self.student_user, stripe_payment_method_id="pm_student_123", card_brand="visa", card_last4="4242"
        )
