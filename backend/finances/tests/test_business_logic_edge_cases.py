...
class FinancialPrecisionTests(BaseAPITestCase):
    ...
    def test_time_based_prorated_refund_precision(self):
        ...
        self.assertEqual(
            daily_rate.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP),
            expected_daily_rate
        )
        self.assertEqual(
            used_amount.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP),
            expected_used
        )
        self.assertEqual(
            refund_amount.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP),
            expected_refund
        )
...
class MultiSchoolPermissionTests(BaseAPITestCase):
    ...
    def test_school_membership_isolation(self):
        ...
        student_a_schools = SchoolMembership.objects.filter(
            user=self.student_a, is_active=True
        ).values_list('school_id', flat=True)
        
        student_b_schools = SchoolMembership.objects.filter(
            user=self.student_b, is_active=True
        ).values_list('school_id', flat=True)
    ...
    def test_serializer_field_completeness(self):
        ...
        for field in required_fields:
            self.assertIn(
                field,
                data,
                f"Payment method serializer missing required field '{field}'"
            )
        ...
        for field in boolean_fields:
            if field in data:
                self.assertIsInstance(
                    data[field], 
                    bool, 
                    f"Field '{field}' should be boolean but got {type(data[field])}"
                )
