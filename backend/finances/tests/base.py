...
class FinanceBaseTestCase(TestCase):
    ...
    def validate_test_fixtures(self):
        ...
        self.assertIsNotNone(self.teacher_compensation_rule.rate_per_hour, "TeacherCompensationRule must have rate_per_hour")
...
class FixtureValidationMixin:
    ...
    def assert_student_has_school_association(self, student_user):
        ...
        self.assertTrue(
            hasattr(student_user, 'student_profile'),
            f"Student {student_user.email} must have a student profile"
        )
    ...
    def assert_teacher_has_profile(self, teacher_user):
        ...
        self.assertTrue(
            hasattr(teacher_user, 'teacher_profile'),
            f"Teacher {teacher_user.email} must have a teacher profile"
        )
        self.assertIsNotNone(
    ...
    def assert_pricing_plan_complete(self, pricing_plan):
        ...
        for field in required_fields:
            ...
            self.assertIsNotNone(
                value,
                f"PricingPlan must have {field} field populated, got {value}"
            )
            
        if pricing_plan.plan_type == 'package':
            self.assertIsNotNone(
                pricing_plan.validity_days,
                "Package pricing plans must have validity_days specified"
            )
    ...
    def assert_student_account_balance_valid(self, student_account_balance):
        ...
        self.assertIsNotNone(
            student_account_balance.student,
            "StudentAccountBalance must have a student association"
        )
