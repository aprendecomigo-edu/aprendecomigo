...
from finances.serializers_admin import (
    ...
    WebhookStatusSummarySerializer
)
...
def payment_metrics(request: Request) -> Response:
    ...
    try:
        ...
        logger.error(f"Error generating payment metrics: {str(e)}", exc_info=True)
        return Response(
...
class WebhookStatusView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if search_term:
            queryset = queryset.filter(
                Q(stripe_event_id__icontains=search_term) |
    ...
    def list(self, request, *args, **kwargs):
        ...
        summary = {
            **webhook_metrics,
            'average_processing_time': processing_metrics.get('average_processing_time', 0.0)
        }
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: WebhookStatusResponseSerializer,
            401: "Authentication required",
            403: "Admin access required"
        },
        tags=['Admin Payment Analytics']
...
def student_analytics(request: Request, student_id: int) -> Response:
    ...
    try:
        ...
        logger.error(f"Error generating student analytics: {str(e)}", exc_info=True)
        return Response(
            {'error': 'Internal server error', 'message': str(e)},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def system_health(request: Request) -> Response:
    ...
    try:
        ...
        logger.error(f"Error in system health check: {str(e)}", exc_info=True)
        return Response(
            {
                'status': 'error',
                'timestamp': timezone.now(),
