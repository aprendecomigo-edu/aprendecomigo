...
class RefundService:
    ...
    def process_refund(
        ...
        try:
            ...
            with transaction.atomic():
                ...
                stripe_result = self._process_stripe_refund(
                ...
                self._update_transaction_for_refund(
                    purchase_transaction, 
                    refund_amount, 
                ...
                self._adjust_student_balance_for_refund(
                    purchase_transaction, 
                    refund_amount
                )
                ...
                logger.info(
                    f"Refund processed successfully for transaction {transaction_id}: €{refund_amount}"
                )
            ...
            if admin_user:
                self._log_admin_action(
                    ...
                    stripe_reference_id=purchase_transaction.stripe_payment_intent_id if 'purchase_transaction' in locals() else None
                )
    ...
    def _process_stripe_refund(
        ...
        try:
            ...
            logger.info(f"Stripe refund created: {refund.id} for payment intent {purchase_transaction.stripe_payment_intent_id}")
    ...
    def _adjust_student_balance_for_refund(
        ...
        """
        try:
            student_balance = StudentAccountBalance.objects.get(
            ...
            if hasattr(purchase_transaction.metadata, 'hours') and purchase_transaction.metadata.get('hours'):
                try:
                    ...
                    logger.warning(f"Could not calculate hours deduction for refund on transaction {purchase_transaction.id}")
            ...
            logger.warning(f"No student balance found for student {purchase_transaction.student.id} during refund processing")
