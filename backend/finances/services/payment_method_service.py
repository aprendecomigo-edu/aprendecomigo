...
class PaymentMethodService:
    ...
    def add_payment_method(
        ...
        try:
            ...
            if StoredPaymentMethod.objects.filter(stripe_payment_method_id=stripe_payment_method_id).exists():
                return {
                    "success": False,
            ...
            with transaction.atomic():
                ...
                if is_default:
                    StoredPaymentMethod.objects.filter(student=student_user, is_default=True).update(is_default=False)
            ...
            logger.error(f"Error adding payment method for student {student_user.id}: {e}", exc_info=True)
            return {"success": False, "error_type": "creation_error", "message": f"Failed to add payment method: {e!s}"}
    ...
    def remove_payment_method(self, student_user, payment_method_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                stored_payment_method = StoredPaymentMethod.objects.get(id=payment_method_id, student=student_user)
            except StoredPaymentMethod.DoesNotExist:
                return {"success": False, "error_type": "not_found", "message": "Payment method not found"}
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    logger.warning(f"Failed to detach payment method {stripe_payment_method_id} from Stripe: {e}")
                ...
                if was_default:
                    next_payment_method = StoredPaymentMethod.objects.filter(
                        student=student_user, is_active=True
                    ).first()
            ...
            return {"success": True, "message": "Payment method removed successfully", "was_default": was_default}

        except Exception as e:
            logger.error(
                f"Error removing payment method {payment_method_id} for student {student_user.id}: {e}", exc_info=True
            )
            return {
                "success": False,
                "error_type": "removal_error",
                "message": f"Failed to remove payment method: {e!s}",
            }
    ...
    def list_payment_methods(self, student_user, include_expired: bool = False) -> dict[str, Any]:
        ...
        """
        try:
            queryset = StoredPaymentMethod.objects.filter(student=student_user, is_active=True).order_by(
            ...
            return {"success": True, "payment_methods": payment_methods, "count": len(payment_methods)}

        except Exception as e:
            logger.error(f"Error listing payment methods for student {student_user.id}: {e}", exc_info=True)
            return {"success": False, "error_type": "list_error", "message": f"Failed to list payment methods: {e!s}"}
    ...
    def set_default_payment_method(self, student_user, payment_method_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": f"Failed to update default payment method: {e!s}",
            }

    def get_default_payment_method(self, student_user) -> StoredPaymentMethod | None:
        """
        ...
        """
        try:
            return StoredPaymentMethod.objects.filter(student=student_user, is_default=True, is_active=True).first()
        except Exception as e:
    ...
    def cleanup_expired_payment_methods(self, student_user=None) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": f"Failed to cleanup expired payment methods: {e!s}",
            }
    ...
    def _validate_stripe_payment_method(self, stripe_payment_method_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "payment_method": card_data, "stripe_data": stripe_pm}
            ...
            return {
                ...
                "message": f"Failed to validate payment method: {e!s}",
            }
    ...
    def _get_or_create_stripe_customer(self, student_user, auto_create: bool = True) -> dict[str, Any]:
        ...
        existing_payment_method = StoredPaymentMethod.objects.filter(
            student=student_user, stripe_customer_id__isnull=False, is_active=True
        ).first()
        ...
        if existing_payment_method and existing_payment_method.stripe_customer_id:
            ...
            if customer_check["success"]:
                return {"success": True, "customer_id": existing_payment_method.stripe_customer_id, "existing": True}
            else:
                logger.warning(
        ...
        try:
            ...
            existing_transaction = PurchaseTransaction.objects.filter(
                student=student_user, stripe_customer_id__isnull=False
            ).first()
        except LookupError:
        ...
        if existing_transaction and existing_transaction.stripe_customer_id:
            ...
            if customer_check["success"]:
                return {"success": True, "customer_id": existing_transaction.stripe_customer_id, "existing": True}
            else:
                logger.warning(
        ...
        if auto_create:
            ...
            customer_result = self.stripe_service.create_customer(
                email=student_user.email, name=student_user.name, metadata=customer_metadata
            )

            if customer_result["success"]:
                logger.info(
                    f"Created new Stripe customer {customer_result['customer_id']} for student {student_user.id}"
                )
