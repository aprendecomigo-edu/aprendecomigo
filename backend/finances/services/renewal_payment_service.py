...
class RenewalPaymentService:
    ...
    def renew_subscription(
        ...
        try:
            ...
            original_transaction = self._get_renewable_transaction(
                student_user, original_transaction_id
            )
            if isinstance(original_transaction, dict):  # Error case
                return original_transaction
            
            # Get payment method
            payment_method = self._get_payment_method_for_renewal(
                student_user, payment_method_id
            )
            ...
            confirmation_result = self._confirm_renewal_payment(
                renewal_intent['payment_intent_id'],
            ...
            logger.error(
                f"Unexpected error during subscription renewal for student {student_user.id}: {e}",
                exc_info=True
            )
            return {
                'success': False,
                'error_type': 'renewal_error',
                'message': f'Subscription renewal failed: {str(e)}'
            }
    ...
    def quick_topup(
        ...
        """
        logger.info(
            f"Starting quick top-up for student {student_user.id}, "
            f"hours: {hours}"
        )
        ...
        try:
            ...
            # Confirm payment immediately for saved payment methods
            confirmation_result = self._confirm_renewal_payment(
                topup_intent['payment_intent_id'],
                payment_method.stripe_payment_method_id
            )
            ...
            logger.error(
                f"Unexpected error during quick top-up for student {student_user.id}: {e}",
                exc_info=True
            )
            return {
                'success': False,
                'error_type': 'topup_error',
                'message': f'Quick top-up failed: {str(e)}'
            }
    ...
    def _get_payment_method_for_renewal(
        ...
        if payment_method_id:
            ...
            payment_method = self.payment_method_service.get_default_payment_method(
                student_user
            )
    ...
    def _create_renewal_payment_intent(
        ...
        try:
            ...
            logger.info(
                f"Created renewal payment intent {payment_intent.id} "
                f"for transaction {new_transaction.id}"
            )
            ...
            return {
                ...
                'message': f'Failed to create renewal payment intent: {str(e)}'
            }
    ...
    def _create_topup_payment_intent(
        ...
        try:
            ...
            return {
                'success': False,
                'error_type': 'payment_intent_creation_error',
                'message': f'Failed to create top-up payment intent: {str(e)}'
            }

    def _confirm_renewal_payment(
        self,
    ...
    def _credit_hours_to_account(self, purchase_transaction: PurchaseTransaction) -> None:
        ...
        if hours_to_add > 0:
            ...
            logger.info(
                f"Added {hours_to_add} hours to account for student {purchase_transaction.student.id}"
            )
    ...
    def _update_account_balance(self, purchase_transaction: PurchaseTransaction) -> None:
        ...
        logger.info(
            f"Added €{purchase_transaction.amount} to balance for student {purchase_transaction.student.id}"
        )
