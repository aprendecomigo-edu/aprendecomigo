...
class PackageExpirationService:
    ...
    def calculate_hours_to_expire(package: PurchaseTransaction) -> Decimal:
        ...
        consumed_hours = HourConsumption.objects.filter(purchase_transaction=package).aggregate(
    ...
    def process_expired_package(package: PurchaseTransaction) -> ExpirationResult:
        ...
        try:
            ...
            error_msg = f"Error processing expired package {package.id}: {e!s}"
    ...
    def send_expiration_warning(
        package: PurchaseTransaction, days_until_expiry: int, respect_preferences: bool = True
    ) -> NotificationResult:
        """
        ...
        try:
            ...
            error_msg = f"Error sending expiration warning: {e!s}"
            logger.error(error_msg)
    ...
    @staticmethod
    def send_expiration_confirmation(package: PurchaseTransaction, hours_expired: Decimal) -> NotificationResult:
        """
        ...
        try:
            ...
            error_msg = f"Error sending expiration confirmation: {e!s}"
            logger.error(error_msg)
    ...
    def send_batch_expiration_warnings(
        packages: list[PurchaseTransaction], days_until_expiry: int
        ...
        for package in packages:
            result = PackageExpirationService.send_expiration_warning(package, days_until_expiry)
            results.append(result)
    ...
    def extend_package_expiration(
        package: PurchaseTransaction, extension_days: int, reason: str = "", extend_from_now: bool = False
    ) -> ExtensionResult:
        """
        ...
        try:
            ...
            error_msg = f"Error extending package {package.id}: {e!s}"
            logger.error(error_msg)
    ...
    def bulk_extend_packages(
        packages: list[PurchaseTransaction], extension_days: int, reason: str = ""
        ...
        results = []
        for package in packages:
            result = PackageExpirationService.extend_package_expiration(package, extension_days, reason)
            results.append(result)
    ...
    def create_package_renewal(
        expired_package: PurchaseTransaction, new_amount: Decimal, new_hours: Decimal, new_validity_days: int
    ) -> RenewalResult:
        """
        ...
        try:
            ...
            new_package = PurchaseTransaction.objects.create(
                ...
                metadata={"hours_included": float(new_hours), "renewed_from": expired_package.id},
            )
            ...
            return RenewalResult(
                success=True, original_package_id=expired_package.id, new_package_id=new_package.id, audit_log=audit_log
            )

        except Exception as e:
            error_msg = f"Error creating package renewal: {e!s}"
            logger.error(error_msg)
    ...
    @staticmethod
    def generate_expiration_summary_report(start_date: datetime, end_date: datetime) -> dict:
        """
        ...
        expiring_soon = packages.filter(
            expires_at__gte=timezone.now(), expires_at__lte=timezone.now() + timedelta(days=30)
        ).count()
        ...
        students_affected = packages.filter(expires_at__lt=timezone.now()).values("student").distinct().count()
    ...
    def calculate_expiration_metrics(period_days: int = 30) -> dict:
        ...
        expiration_rate = expired_packages / total_packages if total_packages > 0 else 0
    ...
    @staticmethod
    def identify_at_risk_students(min_expired_packages: int = 2, timeframe_days: int = 90) -> list[dict]:
        """
