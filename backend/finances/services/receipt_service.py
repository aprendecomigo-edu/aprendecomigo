    ...
    def get_receipt_download_url(cls, receipt_id: int, student_user) -> dict[str, Any]:
        ...
        try:
            ...
            if not receipt.is_valid:
                return {"success": False, "error_type": "invalid_receipt", "message": "This receipt is no longer valid"}
            ...
            return {"success": False, "error_type": "not_found", "message": "Receipt not found"}
        except Exception as e:
            logger.error(f"Error getting download URL for receipt {receipt_id}: {e}", exc_info=True)
            return {"success": False, "error_type": "download_error", "message": f"Failed to get download URL: {e!s}"}

    @classmethod
    def list_student_receipts(cls, student_user, filters: dict | None = None) -> dict[str, Any]:
        """
        ...
        try:
            queryset = (
                Receipt.objects.filter(student=student_user).select_related("transaction").order_by("-generated_at")
            )
            ...
            return {"success": False, "error_type": "list_error", "message": f"Failed to list receipts: {e!s}"}
...
{html_content[html_content.find("<body>") + 6 : html_content.find("</body>")]}
        ...
        logger.info(
            f"Generated HTML content for receipt {receipt.receipt_number} ({len(html_with_styles.encode('utf-8'))} bytes)"
        )
    ...
    def _save_html_file(cls, receipt: Receipt, html_content: str) -> None:
        ...
        filename = f"receipt_{receipt.receipt_number}.html"
        receipt.pdf_file.save(filename, ContentFile(html_content.encode("utf-8")), save=True)
