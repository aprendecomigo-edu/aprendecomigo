...
class HourDeductionService:
    ...
    def validate_and_deduct_hours_for_session(session: ClassSession) -> List[HourConsumption]:
        ...
        with transaction.atomic():
            ...
            for student in students:
                consumption = HourDeductionService._deduct_hours_for_student(
                    student, session, session_hours
                )
                consumption_records.append(consumption)
            
            logger.info(
                f"Successfully deducted {session_hours} hours for {len(students)} students "
        ...
        if not active_packages:
            ...
            if any_packages:
                raise PackageExpiredError(
                    f"Student {student.name} has no active packages (all packages have expired)"
                )
            else:
                raise InsufficientBalanceError(
                    f"Student {student.name} has no tutoring packages purchased"
                )
    ...
    def _deduct_hours_for_student(student, session: ClassSession, hours: Decimal) -> HourConsumption:
        ...
        logger.info(
            f"Deducted {hours} hours from {student.name}'s account "
            f"(transaction {purchase_transaction.id})"
        )
            ...
            for consumption in consumptions:
                ...
                if refunded_hours > Decimal('0.00'):
                    ...
                    refund_summary['students_refunded'].append({
                        'student_name': consumption.student_account.student.name,
                        'hours_refunded': refunded_hours
                    })
    ...
    @staticmethod
    @transaction.atomic
    def deduct_additional_hours_for_session(session: ClassSession, additional_hours: Decimal, reason: str = "") -> List[HourConsumption]:
        """
        ...
        for student in session.students.all():
            ...
            consumption = HourDeductionService._deduct_hours_for_student(
                student, session, additional_hours
            )
            ...
            logger.info(
                f"Deducted additional {additional_hours} hours from {student.name} "
    ...
    @staticmethod
    @transaction.atomic
    def refund_excess_hours_for_session(session: ClassSession, excess_hours: Decimal, reason: str = "") -> List[HourConsumption]:
        """
        ...
        consumptions = HourConsumption.objects.filter(
            session=session,
            is_refunded=False
        ).select_related('student_account__student', 'package')
