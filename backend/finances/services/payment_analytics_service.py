...
class PaymentAnalyticsService:
    ...
    def get_success_rate_metrics(self, hours: int | None = None, days: int | None = None) -> dict[str, Any]:
        """
        ...
        successful_transactions = queryset.filter(payment_status=TransactionPaymentStatus.COMPLETED).count()

        failed_transactions = queryset.filter(payment_status=TransactionPaymentStatus.FAILED).count()

        pending_transactions = queryset.filter(
            payment_status__in=[TransactionPaymentStatus.PENDING, TransactionPaymentStatus.PROCESSING]
        ).count()
    ...
    def get_revenue_trends(self, days: int = 30) -> dict[str, Any]:
        ...
        transactions = PurchaseTransaction.objects.filter(
            payment_status=TransactionPaymentStatus.COMPLETED, created_at__gte=cutoff_date
        ).order_by("created_at")
        ...
        for i in range(days):
            ...
            daily_data.append({"date": current_date, "revenue": daily_revenue.get(current_date, Decimal("0.00"))})
    ...
    def get_revenue_by_type(self, hours: int | None = None) -> dict[str, Decimal]:
        """
        ...
        """
        queryset = PurchaseTransaction.objects.filter(payment_status=TransactionPaymentStatus.COMPLETED)
        ...
        type_breakdown = queryset.values("transaction_type").annotate(revenue=Sum("amount"))
    ...
    def get_transaction_value_metrics(self, hours: int | None = None) -> dict[str, Any]:
        """
    ...
    def get_failure_analysis(self, days: int = 7) -> dict[str, Any]:
        ...
        all_transactions = PurchaseTransaction.objects.filter(created_at__gte=cutoff_date)

        failed_transactions = all_transactions.filter(payment_status=TransactionPaymentStatus.FAILED)
        ...
        for i in range(days):
            current_date = today - timedelta(days=days - 1 - i)
            daily_data.append({"date": current_date, "failures": daily_failures.get(current_date, 0)})
    ...
    def get_common_failure_reasons(self, days: int = 7) -> dict[str, int]:
        ...
        failed_events = WebhookEventLog.objects.filter(created_at__gte=cutoff_date).filter(
            Q(event_type__icontains="failed") | Q(event_type__icontains="declined") | Q(event_type__icontains="error")
        )

        # Count event types
        event_counts = failed_events.values("event_type").annotate(count=Count("id")).order_by("-count")

        return {item["event_type"]: item["count"] for item in event_counts}

    def get_webhook_metrics(self, hours: int | None = None) -> dict[str, Any]:
        """
        ...
        pending_events = status_map.get(WebhookEventStatus.RECEIVED, 0) + status_map.get(
    ...
    def get_webhook_processing_metrics(self, hours: int | None = None) -> dict[str, Any]:
        """
        ...
        """
        queryset = WebhookEventLog.objects.filter(status=WebhookEventStatus.PROCESSED, processed_at__isnull=False)
    ...
    def get_dashboard_metrics(self, hours: int | None = None, days: int | None = None) -> dict[str, Any]:
        """
        ...
        try:
            ...
            metrics = {
                ...
                "time_period": {"hours": time_filter_hours, "days": time_filter_days},
                "payment_success_rate": self.get_success_rate_metrics(hours=time_filter_hours, days=time_filter_days),
            ...
            logger.error(f"Error generating dashboard metrics: {e!s}", exc_info=True)
            raise
    ...
    def _get_recent_activity_summary(self) -> dict[str, Any]:
        ...
        recent_transactions = PurchaseTransaction.objects.filter(created_at__gte=last_24h)

        recent_webhooks = WebhookEventLog.objects.filter(created_at__gte=last_24h)
        ...
        return {
            ...
            "recent_webhook_failures": recent_webhooks.filter(status=WebhookEventStatus.FAILED).count(),
        }
    ...
    def get_student_analytics(self, student_id: int, days: int = 30) -> dict[str, Any]:
        ...
        transactions = PurchaseTransaction.objects.filter(student_id=student_id, created_at__gte=cutoff_date)

        total_spent = transactions.filter(payment_status=TransactionPaymentStatus.COMPLETED).aggregate(
            total=Sum("amount")
        ...
        return {
            ...
            "successful_transactions": transactions.filter(payment_status=TransactionPaymentStatus.COMPLETED).count(),
            "total_spent": total_spent,
            "account_balance": balance_info,
            "transaction_history": list(
                transactions.order_by("-created_at")[:10].values(
