...
from accounts.models import (
    School, SchoolMembership, SchoolRole, StudentProfile, TeacherProfile, 
    ParentProfile, ParentChildRelationship, RelationshipType, EducationalSystem,
    EducationalSystemType, Course, TeacherCourse
)
from finances.models import (
    PricingPlan, PlanType, StudentAccountBalance, PurchaseTransaction, 
    TransactionType, TransactionPaymentStatus, TeacherCompensationRule, 
    CompensationRuleType, ClassSession, SessionType, SessionStatus,
    FamilyBudgetControl, PurchaseApprovalRequest, PurchaseRequestType,
    PurchaseApprovalStatus, StoredPaymentMethod, Receipt, HourConsumption,
    SchoolBillingSettings, TrialCostAbsorption, PaymentFrequency
)
...
class FinanceTestFixtures:
    ...
    def create_minimal_school_setup():
        ...
        educational_system = EducationalSystem.objects.filter(
            code=EducationalSystemType.PORTUGAL
        ).first()
        ...
        school = School.objects.create(
            name='Minimal School',
            description='Minimal test school'
        )
    ...
    def create_parent_child_setup(parent_email="parent@test.com", child_email="child@test.com", school=None):
        ...
        child_membership = SchoolMembership.objects.create(
            user=child_user,
            school=school,
            role=SchoolRole.STUDENT
        )
