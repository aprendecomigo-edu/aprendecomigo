...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if student_email:
                self._process_student_packages(student_email, grace_hours, dry_run, verbosity)
            else:
                self._process_all_packages(grace_hours, dry_run, verbosity)

            # Send notifications if requested
            if send_notifications:
                self._send_expiration_notifications(notification_days, dry_run, verbosity)

        except Exception as e:
            raise CommandError(f"Error processing package expirations: {e}")

    def _process_all_packages(self, grace_hours: int, dry_run: bool, verbosity: int):
        """Process all expired packages."""
        # Get expired packages outside grace period
        expired_packages = PackageExpirationService.get_expired_packages_outside_grace_period(grace_hours=grace_hours)
        ...
        if not expired_packages:
            if verbosity >= 1:
                self.stdout.write(self.style.SUCCESS("No packages need processing"))
            return
    ...
    def _process_student_packages(self, student_email: str, grace_hours: int, dry_run: bool, verbosity: int):
        ...
        if verbosity >= 1:
            self.stdout.write(f"Found {len(expired_packages)} packages for student {student.name}")

        if not expired_packages:
            if verbosity >= 1:
                self.stdout.write(self.style.SUCCESS(f"No packages need processing for {student.name}"))
            return
    ...
    def _send_expiration_notifications(self, notification_days: int, dry_run: bool, verbosity: int):
        ...
        expiring_packages = PackageExpirationService.get_packages_expiring_soon(days_ahead=notification_days)
        ...
        if not expiring_packages:
            if verbosity >= 1:
                self.stdout.write(self.style.SUCCESS("No expiration notifications needed"))
            return
        ...
        results = PackageExpirationService.send_batch_expiration_warnings(
            expiring_packages, days_until_expiry=notification_days
        )
    ...
    def _show_dry_run_results(self, packages, verbosity: int):
        ...
        if verbosity >= 1:
            self.stdout.write(self.style.WARNING("DRY RUN - The following packages would be processed:"))
            ...
            self.stdout.write(f"Total: {len(packages)} packages, {total_hours} hours would be expired")

    def _show_notification_dry_run(self, packages, verbosity: int):
        """Show what notifications would be sent in dry run mode."""
        if verbosity >= 1:
            self.stdout.write(self.style.WARNING("DRY RUN - The following notifications would be sent:"))
    ...
    def _report_processing_results(self, results, verbosity: int):
        ...
        if verbosity >= 1:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully processed {len(successful)} packages ({total_hours_expired} hours expired)"
                )
            )

            if failed:
                self.stdout.write(self.style.ERROR(f"Failed to process {len(failed)} packages"))

        if verbosity >= 2:
            for result in successful:
                self.stdout.write(f"  ✓ Package {result.package_id}: {result.hours_expired}h expired")

            for result in failed:
                self.stdout.write(f"  ✗ Package {result.package_id}: {result.error_message}")
    ...
    def _report_notification_results(self, results, verbosity: int):
        ...
        if verbosity >= 1:
            self.stdout.write(self.style.SUCCESS(f"Successfully sent {len(successful)} notifications"))

            if failed:
                self.stdout.write(self.style.ERROR(f"Failed to send {len(failed)} notifications"))
