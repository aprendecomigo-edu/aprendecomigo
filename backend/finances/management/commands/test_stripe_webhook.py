...
class Command(BaseCommand):
    ...
    help = (
        'Test Stripe webhook integration by creating test data and providing '
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--create-test-data',
            action='store_true',
            help='Create test transaction data for webhook testing'
        )
        
        parser.add_argument(
            '--validate-config',
            action='store_true',
            help='Validate Stripe webhook configuration'
        )
        
        parser.add_argument(
            '--test-endpoint',
            action='store_true',
            help='Test webhook endpoint with mock events'
        )
        
        parser.add_argument(
            '--stripe-cli-instructions',
            action='store_true',
            help='Show instructions for testing with Stripe CLI'
        )
        
        parser.add_argument(
            '--payment-intent-id',
            type=str,
            help='Specific payment intent ID to create test transaction for'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Stripe Webhook Test Command')
        )
    ...
    def _validate_configuration(self):
        ...
        try:
            ...
            if missing_settings:
                self.stdout.write(
                    self.style.ERROR(f"✗ Missing settings: {', '.join(missing_settings)}")
                )
            ...
            if result['success']:
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Stripe API connection successful")
                )
                self.stdout.write(f"  Account ID: {result.get('account_id', 'N/A')}")
            else:
                self.stdout.write(
                    self.style.ERROR(f"✗ Stripe API connection failed: {result.get('message')}")
                )
    ...
    def _create_test_data(self, payment_intent_id: Optional[str] = None):
        ...
        try:
            ...
            if existing_transaction:
                self.stdout.write(
                    f"✓ Using existing transaction with payment intent: {payment_intent_id}"
                )
                ...
                transaction = PurchaseTransaction.objects.create(
                    ...
                    metadata={
                        'hours': '10.00',
                        'package_name': '10-hour test package',
                        'test_transaction': True
                    }
                )
                self.stdout.write(
                    f"✓ Created test transaction with payment intent: {payment_intent_id}"
                )
            ...
            self.stdout.write(
                self.style.ERROR(f"✗ Failed to create test data: {str(e)}")
            )
    ...
    def _test_webhook_endpoint(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"✗ Webhook endpoint testing failed: {str(e)}")
            )
