...
from finances.models import PlanType, PricingPlan
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                if created_count > 0:
                    self.stdout.write(
                        self.style.SUCCESS(f"Successfully created {created_count} default pricing plans.")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING("No new pricing plans were created. Use --force to override existing plans.")
                    )
    ...
    def _create_default_plans(self, force: bool, verbosity: int) -> int:
        ...
        if not force and PricingPlan.objects.exists():
            if verbosity >= 1:
                self.stdout.write(self.style.WARNING("Pricing plans already exist. Use --force to create anyway."))
            return 0
