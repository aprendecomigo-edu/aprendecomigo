...
from ..models import (
    SchoolEmailTemplate, EmailCommunication, 
    EmailTemplateType, EmailCommunicationType, EmailDeliveryStatus
)
from accounts.models import School, CustomUser, TeacherInvitation
...
class EnhancedEmailService:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    email_communication.mark_failed(f"Template rendering failed: {str(template_error)}")
                    raise
                ...
                try:
                    ...
                    email_communication.mark_failed(f"Email sending error: {str(send_error)}")
                    raise
    ...
    def retry_failed_email(cls, email_communication_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Retrying email communication {email_communication_id} (attempt {email_communication.retry_count + 1})")
            ...
            if email_communication.template:
                try:
                    subject, html_content, text_content = EmailTemplateRenderingService.render_template(
                        template=email_communication.template,
                        context_variables=context_variables
                    )
                    ...
                    email_communication.mark_failed(f"Retry error: {str(retry_error)}")
                    return {
                        'success': False,
                        'error': f'Retry error: {str(retry_error)}',
                        'email_communication_id': email_communication_id,
                        'retry_count': email_communication.retry_count
                    }
            else:
            ...
            logger.error(f"Error retrying email communication {email_communication_id}: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'email_communication_id': email_communication_id
...
class EmailAnalyticsService:
    ...
    def get_school_email_stats(cls, school: School, days: int = 30) -> Dict[str, Any]:
        ...
        base_queryset = EmailCommunication.objects.filter(
            school=school,
        ...
        template_type_counts = base_queryset.values('template_type').annotate(
            count=Count('id')
        ).order_by('-count')
