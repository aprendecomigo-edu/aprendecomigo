...
class SecureTemplateEngine:
    ...
    ALLOWED_FILTERS = {
        ...
        'ljust', 'lower', 'make_list', 'pluralize', 'random',
        'removetags', 'rjust', 'safe', 'slice', 'slugify',
        'stringformat', 'striptags', 'time', 'timesince',
        'timeuntil', 'title', 'truncatechars', 'truncatewords',
    ...
    def _is_safe_variable_name(cls, name: str) -> bool:
        ...
        reserved_names = {
            'eval', 'exec', 'open', 'file', 'input', 'compile',
            'globals', 'locals', 'vars', 'dir', 'import'
        }
    ...
    def _sanitize_value(cls, value: Any) -> Any:
        ...
        if isinstance(value, str):
            ...
            return {k: cls._sanitize_value(v) for k, v in value.items() 
                   if cls._is_safe_variable_name(str(k))}
        else:
...
class HTMLSanitizer:
    ...
    def _sanitize_css(cls, html_content: str) -> str:
        ...
        html_content = re.sub(r'style\s*=\s*["\']([^"\']*)["\']', sanitize_style_attribute, html_content, flags=re.IGNORECASE)
