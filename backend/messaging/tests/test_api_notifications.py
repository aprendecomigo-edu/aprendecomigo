...
class NotificationAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.school = School.objects.create(
            name="Test School", description="A test school", address="123 Test Street, Test City"
        )

        # Create test users
        self.student = CustomUser.objects.create_user(email="student@test.com", name="Test Student")

        self.other_student = CustomUser.objects.create_user(email="other@test.com", name="Other Student")

        # Create school memberships
        SchoolMembership.objects.create(user=self.student, school=self.school, role=SchoolRole.STUDENT)

        SchoolMembership.objects.create(user=self.other_student, school=self.school, role=SchoolRole.STUDENT)
    ...
    def create_notification(
        ...
        return Notification.objects.create(
            user=user, notification_type=notification_type, title=title, message=message, is_read=is_read
        )
...
class NotificationListAPITest(NotificationAPITestCase):
    ...
    def test_list_user_notifications(self):
        ...
        **Response Format:**
        ...
        notification1 = self.create_notification(title="Low Balance Alert", message="Your balance is running low")
        ...
        self.create_notification(
            user=self.other_student, title="Other Student Notification", message="This should not appear"
        )
    ...
    def test_list_notifications_pagination(self):
        ...
        for i in range(25):
            self.create_notification(title=f"Notification {i}", message=f"Message {i}")
    ...
    def test_list_notifications_filter_by_type(self):
        ...
        low_balance = self.create_notification(notification_type=NotificationType.LOW_BALANCE, title="Low Balance")
        ...
        depleted = self.create_notification(
            notification_type=NotificationType.BALANCE_DEPLETED, title="Balance Depleted"
        )
    ...
    def test_list_notifications_filter_by_read_status(self):
        ...
        read_notification = self.create_notification(title="Read Notification", is_read=True)
        read_notification.read_at = timezone.now()
        read_notification.save()

        unread_notification = self.create_notification(title="Unread Notification", is_read=False)
...
class NotificationDetailAPITest(NotificationAPITestCase):
    ...
    def test_get_notification_detail_other_user(self):
        ...
        other_notification = self.create_notification(
            user=self.other_student, title="Other User Notification", message="Should not be accessible"
        )
...
class NotificationMarkAsReadAPITest(NotificationAPITestCase):
    ...
    def test_mark_notification_as_read(self):
        ...
        **Side Effects:**
        ...
        """
        notification = self.create_notification(title="Test Notification", message="Test message", is_read=False)
    ...
    def test_mark_already_read_notification(self):
        ...
        notification = self.create_notification(title="Already Read", message="Already read message", is_read=True)
...
class NotificationUnreadCountAPITest(NotificationAPITestCase):
    ...
    def test_get_unread_count_with_notifications(self):
        ...
        **Business Logic:**
        ...
        self.create_notification(user=self.other_student, title="Other User Unread", is_read=False)
    ...
    def test_get_unread_count_all_read(self):
        ...
        # Create read notifications
        for i in range(5):
            notification = self.create_notification(title=f"Read Notification {i}", is_read=True)
            notification.read_at = timezone.now()
            notification.save()
...
class NotificationAPIIntegrationTest(NotificationAPITestCase):
    ...
    def test_notification_api_workflow(self):
        ...
        **Business Scenario:**
        ...
        notification1 = self.create_notification(
            title="Balance Low", message="Your balance is running low", notification_type=NotificationType.LOW_BALANCE
        )
    ...
    def test_notification_metadata_support(self):
        ...
        metadata = {"remaining_hours": 1.5, "expiry_date": "2025-08-08", "package_type": "Premium"}
