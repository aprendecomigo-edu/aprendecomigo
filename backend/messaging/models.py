...
class Notification(models.Model):
    ...
    message = models.TextField(_("message"), help_text=_("Notification message content"))

    is_read = models.BooleanField(_("is read"), default=False, help_text=_("Whether the notification has been read"))

    read_at = models.DateTimeField(
        _("read at"), null=True, blank=True, help_text=_("Timestamp when notification was read")
    )
    ...
    metadata = models.JSONField(
        _("metadata"), default=dict, blank=True, help_text=_("Additional notification data in JSON format")
    )
...
class SchoolEmailTemplate(models.Model):
    ...
    html_content = models.TextField(_("HTML content"), help_text=_("HTML email content with template variables"))
    text_content = models.TextField(_("text content"), help_text=_("Plain text email content with template variables"))

    # Branding and customization
    use_school_branding = models.BooleanField(
        _("use school branding"), default=True, help_text=_("Apply school colors and logo to template")
    )
    custom_css = models.TextField(
        _("custom CSS"), blank=True, null=True, help_text=_("Additional CSS for template customization")
    )

    # Metadata
    is_active = models.BooleanField(
        _("is active"), default=True, help_text=_("Whether this template is active and can be used")
    )
    is_default = models.BooleanField(
        _("is default"), default=False, help_text=_("Whether this is the default template for this type")
    )
    ...
    def clean(self):
        ...
        if self.is_default:
            existing_default = self.__class__.objects.filter(
                school=self.school, template_type=self.template_type, is_default=True
            ).exclude(pk=self.pk)

            if existing_default.exists():
                raise ValidationError(_("Only one default template per school and type is allowed"))
    ...
    def _validate_template_security(self):
        ...
        try:
            ...
            raise ValidationError(_("Template security validation failed: %(error)s") % {"error": str(e)})
    ...
    def _validate_custom_css_security(self):
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, css_lower):
                raise ValidationError(_("Custom CSS contains dangerous pattern: %(pattern)s") % {"pattern": pattern})
...
class EmailSequence(models.Model):
    ...
    name = models.CharField(_("sequence name"), max_length=200, help_text=_("Human-readable name for the sequence"))
    description = models.TextField(
        _("description"), blank=True, null=True, help_text=_("Description of what this sequence does")
    )
    ...
    is_active = models.BooleanField(_("is active"), default=True, help_text=_("Whether this sequence is active"))
    max_emails = models.PositiveIntegerField(
        _("maximum emails"), default=5, help_text=_("Maximum number of emails to send in this sequence")
    )
...
class EmailSequenceStep(models.Model):
    ...
    # Step configuration
    step_number = models.PositiveIntegerField(_("step number"), help_text=_("Order of this step in the sequence"))
    delay_hours = models.PositiveIntegerField(
        _("delay hours"), default=24, help_text=_("Hours to wait before sending this email")
    )
    ...
    is_active = models.BooleanField(_("is active"), default=True, help_text=_("Whether this step is active"))
...
class EmailCommunication(models.Model):
    ...
    recipient_email = models.EmailField(_("recipient email"), help_text=_("Email address of the recipient"))
    school = models.ForeignKey(
        "accounts.School",
        on_delete=models.CASCADE,
    ...
    failure_reason = models.TextField(
        _("failure reason"), blank=True, null=True, help_text=_("Reason for delivery failure")
    )
    retry_count = models.PositiveIntegerField(_("retry count"), default=0, help_text=_("Number of delivery attempts"))
    max_retries = models.PositiveIntegerField(
        _("maximum retries"), default=3, help_text=_("Maximum number of retry attempts")
    )
    ...
    def can_retry(self):
        ...
        return self.delivery_status == EmailDeliveryStatus.FAILED and self.retry_count < self.max_retries
