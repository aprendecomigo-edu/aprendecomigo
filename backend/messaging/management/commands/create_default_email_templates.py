...
from messaging.services.teacher_communication_templates import DefaultEmailTemplates
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--school-id", type=int, help="Create templates for a specific school ID")
        parser.add_argument(
            "--all-schools", action="store_true", help="Create templates for all schools that don't have them"
        )
            ...
            if school_id:
                ...
                for school in schools:
                    try:
                        ...
                        self.stdout.write(self.style.ERROR(f"Error creating templates for {school.name}: {e!s}"))
                        continue

        except School.DoesNotExist:
            raise CommandError(f"School with ID {school_id} does not exist")
        except Exception as e:
            raise CommandError(f"Error creating templates: {e!s}")
        ...
        for template_type in template_types:
            try:
                ...
                existing_template = SchoolEmailTemplate.objects.filter(
                    school=school, template_type=template_type
                ).first()
                ...
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f"  - Skipped {template_type}: {e!s}"))
                    continue
                ...
                self.stdout.write(self.style.ERROR(f"  - Error creating {template_type}: {e!s}"))
                continue

        self.stdout.write(f"  Summary: {created_count} created, {updated_count} updated, {skipped_count} skipped")
    ...
    def create_global_default_templates(self):
        ...
        for template_type_tuple in EmailTemplateType.choices:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"  - Error creating global {template_type}: {e!s}"))
                continue
