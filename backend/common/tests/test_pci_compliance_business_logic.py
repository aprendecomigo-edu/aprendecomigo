...
class PCIValidationRulesTest(TestCase):
    ...
    def test_validate_pci_compliance_safe_strings(self):
        ...
        for safe_string in safe_strings:
            if safe_string is not None:
                self.assertTrue(validate_pci_compliance(safe_string), 
                              f"'{safe_string}' should be PCI compliant")
    ...
    def test_validate_pci_compliance_cvv_patterns(self):
        ...
        for cvv in cvv_patterns:
            self.assertFalse(validate_pci_compliance(cvv), 
                           f"CVV pattern '{cvv}' should not be PCI compliant")
    ...
    def test_validate_pci_compliance_full_card_patterns(self):
        ...
        for card in card_patterns:
            self.assertFalse(validate_pci_compliance(card), 
                           f"Card pattern '{card}' should not be PCI compliant")
    ...
    def test_validate_pci_compliance_test_card_detection(self):
        ...
        for test_card in test_cards:
            self.assertFalse(validate_pci_compliance(test_card), 
                           f"Test card '{test_card}' should not be PCI compliant")
    ...
    def test_validate_pci_compliance_edge_cases(self):
        ...
        for test_string, expected in edge_cases:
            ...
            self.assertEqual(result, expected, 
                           f"'{test_string}' validation should be {expected}")
...
class SecurityPatternDetectionTest(TestCase):
    ...
    def test_pattern_detection_common_violations(self):
        ...
        for pattern, description in violation_patterns:
            self.assertFalse(validate_pci_compliance(pattern), 
                           f"{description} should be detected as violation: {pattern}")
    ...
    def test_pattern_detection_false_positives(self):
        ...
        for pattern in valid_patterns:
            self.assertTrue(validate_pci_compliance(pattern), 
                          f"Valid pattern should not be flagged: {pattern}")
    ...
    def test_pattern_detection_boundary_conditions(self):
        ...
        for digits, expected_safe in boundary_cases:
            ...
            self.assertEqual(result, expected_safe, 
                           f"{len(digits)}-digit pattern '{digits}' validation should be {expected_safe}")
...
class AuditTrailComplianceTest(TestCase):
    ...
    def test_masking_creates_audit_safe_data(self):
        ...
        for raw_input in raw_inputs:
            ...
            self.assertTrue(validate_pci_compliance(masked), 
                          f"Masked data should be audit-safe: {masked}")
            
            # Should not contain original raw digits
            self.assertNotEqual(masked, raw_input, 
                              f"Masked data should not equal raw input: {raw_input}")
