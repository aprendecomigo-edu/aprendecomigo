...
ALLOWED_HTML_TAGS_RICH_TEXT = {"p", "br", "strong", "em", "b", "i", "u"}
...
def sanitize_name_field(value: str | None) -> str | None:
    """
...
def sanitize_text_content(value: str | None) -> str | None:
    """
    ...
    sanitized = re.sub(
        r'\b(?:src|href|action|formaction|data)\s*=\s*["\']?(?:javascript|vbscript|data|file|ftp):[^"\'>\s]*["\']?',
        "",
        sanitized,
        flags=re.IGNORECASE,
    )
    ...
    sanitized = re.sub(
        r'url\s*\(\s*["\']?(?:javascript|vbscript|data|file):[^"\')\s]*["\']?\s*\)', "", sanitized, flags=re.IGNORECASE
    )
...
def sanitize_multiline_text(value: str | None) -> str | None:
    ...
    sanitized = re.sub(
        r'data:text/html,[^"\'>\s]*<script[^>]*>.*?</script>[^"\'>\s]*', "", value, flags=re.IGNORECASE | re.DOTALL
    )
    ...
    sanitized = re.sub(
        r"^javascript:(alert|eval|confirm|prompt|setTimeout|setInterval|Function)\s*\([^)]*\)\s*$",
        "",
        sanitized,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    sanitized = re.sub(
        r"^vbscript:(msgbox|eval|execute|executeglobal)\s*\([^)]*\)\s*$",
        "",
        sanitized,
        flags=re.IGNORECASE | re.MULTILINE,
    )
    ...
    sanitized = bleach.clean(
        sanitized, tags=ALLOWED_HTML_TAGS_PLAIN_TEXT, attributes=ALLOWED_HTML_ATTRIBUTES, strip=True
    )
