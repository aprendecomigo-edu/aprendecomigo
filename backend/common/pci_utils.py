...
def mask_card_last4(card_last4: Optional[str]) -> Optional[str]:
    ...
    Returns:
        ...
        Returns None if input is None, empty, or whitespace-only
    """
...
def validate_pci_compliance(value: str) -> bool:
    ...
    - CVV patterns in text (e.g., "CVV: 123")
    - Exactly 16-digit sequences (full card numbers)
    ...
    # Check for CVV patterns in text (case-insensitive)
    # Matches patterns like "CVV: 123", "cvv:123", "CVV 123", etc.
    if re.search(r'\bcvv\s*:?\s*\d{3,4}\b', value_str, re.IGNORECASE):
        return False
        
    # Check for full card number patterns (exactly 16 digits as word boundary)
    if re.search(r'\b\d{16}\b', value_str):
        return False
