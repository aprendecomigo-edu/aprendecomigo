...
from .models import (
    ClassSchedule, ClassStatus, ClassType,
    ClassReminder, ReminderPreference, ReminderType, 
    CommunicationChannel, ReminderStatus
)
...
class CommunicationPayloadService:
    ...
    def generate_reminder_payload(class_schedule, reminder_type, recipients, context_data=None, locale="en"):
        ...
        if reminder_type in [ReminderType.REMINDER_24H, ReminderType.REMINDER_1H, ReminderType.REMINDER_15MIN]:
            hours_map = {
                ReminderType.REMINDER_24H: 24,
                ReminderType.REMINDER_1H: 1,
                ReminderType.REMINDER_15MIN: 0.25
            }
...
class ReminderBackgroundTaskService:
    ...
    def find_classes_needing_reminders(reminder_type, tolerance_minutes=30):
        ...
        if len(classes_in_window) > 1:
            ...
            result_classes = [
                cls for cls in classes_in_window 
                if cls.id not in existing_reminder_class_ids
            ]
        else:
        ...
        for batch_num in range(max_batches):
            ...
            for reminder in batch_reminders:
                try:
                    ...
                    result['total_failed'] += 1
                    result['errors'].append(f"Reminder {reminder.id}: {str(e)}")
    ...
    def get_queue_health_status():
        ...
        last_processed = ClassReminder.objects.filter(
    ...
    def generate_reminders_for_user(class_schedule, user, role=None):
        ...
        try:
            ...
            for hours_before in prefs.reminder_timing_hours:
                ...
                valid_channels = [
                    ch for ch in prefs.communication_channels 
...
class ReminderService:
    ...
    def should_send_reminder(class_schedule, reminder_type, recipient):
        ...
        return DuplicateReminderPreventionService.should_send_reminder(
            class_schedule, reminder_type, recipient
        )
    
    @staticmethod
    def cancel_pending_reminders(class_schedule, reason="Class cancelled"):
        """Cancel pending reminders (delegates to DuplicateReminderPreventionService)."""
        return DuplicateReminderPreventionService.cancel_pending_reminders(
            class_schedule, reason
        )
    ...
    def handle_class_status_change(class_schedule, old_status, new_status, changed_by):
        ...
        if new_status == ClassStatus.CANCELLED:
            DuplicateReminderPreventionService.cancel_pending_reminders(
                class_schedule, "Class was cancelled"
            )
    ...
    def send_reminder(reminder):
        ...
        if hasattr(reminder, 'subject') and hasattr(reminder, 'message'):
            try:
                (sanitized_subject, sanitized_message), validation_errors = TimezoneValidationService.sanitize_message_content(
                ...
                reminder.mark_failed(f"Message sanitization error: {str(e)}")
                return False
