...
class ClassScheduleViewSet(SchoolPermissionMixin, viewsets.ModelViewSet):
    ...
    def get_object_for_permissions(self):
        ...
        try:
            ...
            ).prefetch_related("additional_students").get(pk=pk)
    ...
    def no_show(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"message": "Class marked as no-show successfully"}, 
                status=status.HTTP_200_OK
            )
        except PermissionDenied as e:
            return Response(
    ...
    def reminders(self, request, pk=None):
        ...
        reminders = ClassReminder.objects.filter(
    ...
    def trigger_reminder(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Reminder services not available"},
        ...
        recipients = RecipientDeterminationService.determine_recipients(
            class_schedule, reminder_type
        )
            ...
            for channel in channels:
                try:
                    reminder = ClassReminder.objects.create(
                        ...
                        subject=custom_subject or f"Class {reminder_type.replace('_', ' ').title()}: {class_schedule.title}",
        ...
        response_data = {
            ...
            "communication_status": "success" if failed_count == 0 else ("failed" if reminders_sent == 0 else "partial_failure")
        }
...
class RecurringClassScheduleViewSet(SchoolPermissionMixin, viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        ...
        instance.cancel_series(
            reason="Series deleted via API",
            cancelled_by=user
        )
    ...
    def cancel_instance(self, request, pk=None):
        ...
        try:
            result = recurring_schedule.cancel_occurrence(
                date=date,
                reason=reason,
...
class ReminderPreferenceViewSet(SchoolPermissionMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ReminderPreference.objects.filter(
            user=self.request.user
        ).select_related("user", "school").order_by("school", "created_at")
...
class ClassReminderViewSet(SchoolPermissionMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = ClassReminder.objects.filter(
            class_schedule__school__in=user_schools
        ).select_related(
            "class_schedule__teacher__user",
            "class_schedule__student", 
            "class_schedule__school",
            "recipient"
        )
        ...
        if search:
            queryset = queryset.filter(
                Q(class_schedule__title__icontains=search) |
...
class UserRemindersViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ClassReminder.objects.filter(
