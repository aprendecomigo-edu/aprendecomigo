...
from accounts.models import CustomUser, School, TeacherProfile, SchoolSettings
...
class BookingValidationService:
    ...
    def validate_class_type_consistency(self, class_type: str, max_participants: Optional[int] = None) -> bool:
...
class TeacherAvailabilityService:
    ...
    def is_teacher_available(self, teacher: TeacherProfile, school: School, 
        ...
        return (start_time >= availability.start_time and 
                end_time <= availability.end_time)
    
    def get_teacher_availability_for_date(self, teacher: TeacherProfile, 
                                        school: School, date) -> Optional[TeacherAvailability]:
...
class GroupClassCapacityService:
    ...
    def find_joinable_group_class(self, teacher: TeacherProfile, school: School,
                                date, start_time: time, end_time: time,
                                max_participants: int) -> Optional[ClassSchedule]:
    ...
    def can_student_join_class(self, class_schedule: ClassSchedule, 
                              student: CustomUser) -> bool:
    ...
    def add_student_to_group_class(self, class_schedule: ClassSchedule, 
                                  student: CustomUser, booked_by: CustomUser) -> ClassSchedule:
...
class MinimumNoticeService:
    ...
    def meets_minimum_notice_period(self, booking_date, booking_time: time, 
                                  school: School, minimum_hours: int = None) -> bool:
    ...
    def get_hours_until_minimum_notice_met(self, booking_date, booking_time: time,
                                         school: School, minimum_hours: int) -> float:
...
class BookingOrchestratorService:
    ...
    def __init__(self):
        ...
        from .scheduling_rules_services import SchedulingRulesService, ConflictDetectionService
        ...
        try:
            ...
            if not self.availability_service.is_teacher_available(teacher, school, date, start_time, end_time):
                ...
                if not availability:
                    ...
                    errors.append(f"Class time must be within teacher's available hours ({availability.start_time} - {availability.end_time})")
                ...
                if any(p['is_all_day'] for p in periods):
                    ...
                    for period in periods:
                        if not period['is_all_day']:
                            errors.append(f"Teacher is unavailable from {period['start_time']} to {period['end_time']} ({period['reason']})")
            ...
            if class_type == ClassType.GROUP and max_participants:
                ...
                for existing_group in existing_group_classes:
                    if not self.group_service.can_student_join_class(existing_group, student):
                        if existing_group.student == student or existing_group.additional_students.filter(id=student.id).exists():
            ...
            try:
                ...
                self.scheduling_rules_service.validate_minimum_notice(
                    booking_datetime, school, teacher, class_type
                )
            ...
            try:
                self.scheduling_rules_service.validate_booking_limits(
                    student, teacher, school, date, class_type
                )
            except ValidationError as e:
                errors.append(str(e))
        ...
        if class_type == ClassType.GROUP:
            ...
            if existing_class:
                ...
                if self.group_service.can_student_join_class(existing_class, student):
                    updated_class = self.group_service.add_student_to_group_class(
                        existing_class, student, booked_by
                    )
                    return {
                        'class_schedule': updated_class,
                        'action': 'joined_existing_group_class'
                    }
                else:
...
class AvailableSlotsService:
    ...
    def get_available_slots(self, start_date, duration_minutes: int, end_date=None, 
                          class_type: str = None, requesting_student = None):
        ...
        if len(self.schools) == 1:
            ...
            result = enhanced_service.get_available_slots_with_rules(
                self.teacher, school, start_date, duration_minutes, 
                end_date, class_type, requesting_student
            )
    ...
    def _get_slots_for_date(self, date, duration_minutes: int):
        ...
        for availability in availabilities:
            school_slots = self._generate_slots_for_availability(
                date, availability, duration_minutes
            )
    ...
    def _generate_slots_for_availability(self, date, availability, duration_minutes: int):
        ...
        while current_time < end_limit:
            ...
            # Check for conflicts
            if not self._has_conflicts(date, current_time, slot_end_time, 
                                     unavailabilities, existing_classes):
            ...
            current_time = (datetime.combine(date, current_time) + timedelta(minutes=DEFAULT_SLOT_INTERVAL_MINUTES)).time()
    ...
    def _has_conflicts(self, date, start_time, end_time, unavailabilities, existing_classes):
        ...
        for unavailability in unavailabilities:
            ...
            if (unavailability.start_time and unavailability.end_time and
                overlap_detector.has_time_overlap(
                    start_time, end_time,
        ...
        for class_schedule in existing_classes:
            if overlap_detector.has_time_overlap(
                start_time, end_time,
                class_schedule.start_time, class_schedule.end_time
            ):
                return True
...
class ClassStatusTransitionService:
    ...
    def confirm_class(self, class_schedule: 'ClassSchedule', confirmed_by: CustomUser) -> Dict[str, Any]:
        ...
        if not self.can_transition_to_confirmed(class_schedule):
            ...
            elif class_schedule.status in [ClassStatus.CANCELLED, ClassStatus.COMPLETED, ClassStatus.NO_SHOW, ClassStatus.REJECTED]:
    ...
    def cancel_class(self, class_schedule: 'ClassSchedule', cancelled_by: CustomUser, reason: str = "") -> Dict[str, Any]:
        """Cancel a class."""
        from .models import ClassStatus
    ...
    def complete_class(self, class_schedule: 'ClassSchedule', completed_by: CustomUser) -> Dict[str, Any]:
        ...
        if not self.can_transition_to_completed(class_schedule):
            if class_schedule.status == ClassStatus.COMPLETED:
                ...
                raise ValidationError("Only confirmed classes can be marked as completed - this class is in a final state")
            else:
...
class ClassPermissionService:
    ...
    def can_cancel_class(self, class_schedule: 'ClassSchedule', user: CustomUser) -> bool:
        ...
        if membership.role == SchoolRole.STUDENT:
            return (class_schedule.student == user or 
                   class_schedule.additional_students.filter(id=user.id).exists())
...
class ClassMetadataTrackingService:
    ...
    def track_cancellation(self, class_schedule: 'ClassSchedule', cancelled_by: CustomUser, reason: str = "") -> Dict[str, Any]:
...
class CancellationDeadlineService:
    ...
    def can_cancel_within_deadline(self, class_schedule: 'ClassSchedule', hours_before: int = None) -> bool:
        ...
        if class_schedule.status in [ClassStatus.COMPLETED, ClassStatus.CANCELLED, ClassStatus.NO_SHOW, ClassStatus.REJECTED]:
            return False
...
class ClassCompletionService:
    ...
    def mark_as_completed(self, class_schedule: 'ClassSchedule', completed_by: CustomUser,
                         actual_duration_minutes: int = None, notes: str = "") -> Dict[str, Any]:
        ...
        if actual_duration_minutes is not None:
            ...
            if actual_duration_minutes > MAX_ACTUAL_DURATION_MINUTES:
                raise ValidationError(f"Actual duration must be reasonable (maximum {MAX_ACTUAL_DURATION_MINUTES//60} hours)")
        else:
...
class ClassNoShowService:
    ...
    def mark_as_no_show(self, class_schedule: 'ClassSchedule', marked_by: CustomUser,
                       reason: str, no_show_type: str = "student", notes: str = "") -> Dict[str, Any]:
...
class ClassCompletionOrchestratorService:
    ...
    def complete_class(self, class_schedule: 'ClassSchedule', user: CustomUser,
                      actual_duration_minutes: int = None, notes: str = "") -> Dict[str, Any]:
        ...
        result = self.completion_service.mark_as_completed(
            class_schedule, user, actual_duration_minutes, notes
        )
        
        return result
    
    def mark_no_show(self, class_schedule: 'ClassSchedule', user: CustomUser,
                    reason: str, no_show_type: str = "student", notes: str = "") -> Dict[str, Any]:
        ...
        result = self.no_show_service.mark_as_no_show(
            class_schedule, user, reason, no_show_type, notes
        )
