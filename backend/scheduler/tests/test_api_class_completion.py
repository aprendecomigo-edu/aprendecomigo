...
class ClassCompletionTests(SchedulerAPITestCase):
    ...
    def test_completion_with_optional_data(self):
        ...
        test_cases = [
            {"data": {"actual_duration_minutes": 55}, "expected_duration": 55, "description": "with actual duration"},
            {
    ...
    def test_no_show_with_different_types(self):
        ...
        for no_show_type in ["student", "teacher"]:
            with self.subTest(no_show_type=no_show_type):
                ...
                data = {"reason": f"{no_show_type.title()} no-show", "no_show_type": no_show_type}
                response = self.client.post(
                    reverse("class-schedules-no-show", kwargs={"pk": test_class.id}), data, format="json"
                )
