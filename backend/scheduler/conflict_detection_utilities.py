...
def detect_student_conflicts(student_id: int, school_id: int, booking_date: date,
...
def detect_teacher_conflicts(teacher_id: int, school_id: int, booking_date: date,
                           start_time: time, end_time: time, buffer_minutes: int = 15,
                           exclude_class_id: int = None) -> Optional[Dict[str, Any]]:
    """
    ...
    return _detect_buffer_conflicts(
        teacher_id, school_id, booking_date, start_time, end_time, 
        buffer_minutes, exclude_class_id
    )


def _detect_buffer_conflicts(teacher_id: int, school_id: int, booking_date: date,
                           start_time: time, end_time: time, buffer_minutes: int,
                           exclude_class_id: int = None) -> Optional[Dict[str, Any]]:
...
def _has_buffer_time_conflict(date1: date, start1: time, end1: time,
...
def _calculate_earliest_available_after_class(conflict_date: date, conflict_end_time: time, 
...
def detect_time_overlap(start1: time, end1: time, start2: time, end2: time,
    ...
    if has_overlap:
        ...
        result.update({
            ...
            'overlap_minutes': int(overlap_duration)
        })
    else:
        result.update({
            'overlap_start': None,
...
def find_available_gaps(schedule: List[Dict], target_duration_minutes: int, 
                       working_start: time = time(8, 0), working_end: time = time(18, 0),
    ...
    if not schedule:
        ...
        return [{
            ...
            'duration_minutes': int((
                datetime.combine(date.today(), working_end) -
                datetime.combine(date.today(), working_start)
            ).total_seconds() / 60)
        }]
    ...
    for cls in sorted_schedule:
        ...
        if buffer_minutes > 0:
            buffered_start = (
                datetime.combine(date.today(), cls_start) - 
                timedelta(minutes=buffer_minutes)
            ).time()
            
            buffered_end = (
                datetime.combine(date.today(), cls_end) + 
                timedelta(minutes=buffer_minutes)
            ).time()
        else:
        ...
        if current_time < buffered_start:
            gap_duration = (
                datetime.combine(date.today(), buffered_start) -
...
def validate_time_boundaries(start_time: time, end_time: time, 
                           max_duration_minutes: int = 480) -> Dict[str, Any]:
    """
