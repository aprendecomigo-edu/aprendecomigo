...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Error creating calendar events: {e!s}"))
            raise e
    ...
    def get_admin_user(self, email):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Found admin user: {email}"))
            return user
    ...
    def get_admin_school(self, admin_user):
        ...
        if not admin_membership:
            self.stdout.write(
                self.style.ERROR(
                    "No school membership found for admin user. Please run create_school_admin_test_data first."
                )
            )
            raise Exception("Admin user has no school membership")

        self.stdout.write(self.style.SUCCESS(f"Using school: {admin_membership.school.name}"))
    ...
    def clear_existing_admin_events(self, admin_user, school):
        ...
        self.stdout.write(self.style.WARNING(f"Cleared {deleted_count} existing admin calendar events"))
    ...
    def create_admin_calendar_events(self, admin_user, school):
        ...
        for event_data in events_data:
            ...
            end_time = (datetime.combine(scheduled_date, start_time) + timedelta(minutes=event_data["duration"])).time()
            ...
            if event_data["status"] == ClassStatus.COMPLETED:
                event.completed_at = timezone.make_aware(datetime.combine(scheduled_date, end_time))
                event.save()
    ...
    def get_or_create_admin_teacher(self, school):
        ...
        teacher_profile, created = TeacherProfile.objects.get_or_create(
            ...
            defaults={
                ...
                "hourly_rate": 0,
                "teaching_experience": {"years_total": 0, "specializations": ["Administração"]},
