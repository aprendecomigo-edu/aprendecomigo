...
class SchedulingConfigurationService:
    ...
    def get_minimum_notice_hours(self, school: School, teacher: TeacherProfile = None, 
                               class_type: str = None) -> int:
    ...
    def get_minimum_notice_minutes(self, school: School, teacher: TeacherProfile = None, 
                                 class_type: str = None) -> int:
    ...
    def get_buffer_time_minutes(self, school: School, teacher: TeacherProfile = None, 
                               class_type: str = None) -> int:
    ...
    def get_daily_booking_limit(self, school: School, teacher: TeacherProfile = None, 
                               is_student: bool = False) -> int:
    ...
    def get_weekly_booking_limit(self, school: School, teacher: TeacherProfile = None, 
                                is_student: bool = False) -> int:
    ...
    def get_rule_configuration(self, school: School, teacher: TeacherProfile = None, 
                              class_type: str = None, is_student: bool = False) -> Dict[str, Any]:
...
class SchedulingRulesService:
    ...
    def validate_minimum_notice(self, booking_datetime: datetime, school: School, 
                               teacher: TeacherProfile = None, class_type: str = None) -> bool:
    ...
    def calculate_buffer_time(self, class_type: str = None, teacher: TeacherProfile = None, 
                             school: School = None) -> int:
    ...
    def validate_booking_limits(self, student: CustomUser, teacher: TeacherProfile, school: School,
                               booking_date: date, class_type: str = None) -> bool:
...
class ConflictDetectionService:
    ...
    def detect_student_conflicts(self, student: CustomUser, school: School, booking_date: date, 
                               start_time: time, end_time: time, 
                               exclude_class_id: int = None) -> Optional[Dict[str, Any]]:
        ...
        all_school_conflicts = ClassSchedule.objects.filter(
            Q(student=student) | Q(additional_students=student),
            scheduled_date=booking_date,
            status__in=[ClassStatus.SCHEDULED, ClassStatus.CONFIRMED]
        ).exclude(
    ...
    def detect_teacher_conflicts(self, teacher: TeacherProfile, school: School, booking_date: date, 
                               start_time: time, end_time: time, class_type: str = None,
                               exclude_class_id: int = None) -> Optional[Dict[str, Any]]:
        ...
        buffer_start_time = buffer_start_datetime.time()
        buffer_end_time = buffer_end_datetime.time()
        ...
        for search_date in search_dates:
            ...
            for existing_class in date_conflicts:
                if self._has_buffer_conflict(
                    ...
                    existing_class.scheduled_date, existing_class.start_time, existing_class.end_time,
                    buffer_minutes
                ):
    ...
    def _has_buffer_conflict(self, date1: date, start1: time, end1: time,
                           date2: date, start2: time, end2: time, buffer_minutes: int) -> bool:
    ...
    def _calculate_earliest_available_time(self, conflict_date: date, conflict_end_time: time, 
                                         buffer_minutes: int) -> str:
    ...
    def detect_time_overlap(self, start1: time, end1: time, start2: time, end2: time, 
                           return_details: bool = False) -> Union[bool, Dict[str, Any]]:
    ...
    def handle_day_boundary_cases(self, booking_date: date, start_time: time, 
                                 end_time: time) -> Dict[str, Any]:
...
class AvailabilityCalculationService:
    ...
    def get_available_slots_with_rules(self, teacher: TeacherProfile, school: School,
                                     start_date: date, duration_minutes: int,
                                     end_date: date = None, class_type: str = None,
                                     requesting_student: CustomUser = None) -> Dict[str, Any]:
        ...
        while current_date <= end_date:
            day_slots = self._get_slots_for_date_with_rules(
                teacher, school, current_date, duration_minutes, 
                class_type, requesting_student, config
            )
    ...
    def _get_slots_for_date_with_rules(self, teacher: TeacherProfile, school: School, 
                                     booking_date: date, duration_minutes: int,
                                     class_type: str = None, requesting_student: CustomUser = None,
        ...
        while current_time < end_limit:
            ...
            if self._has_slot_conflicts(teacher, school, booking_date, current_time, 
                                      slot_end_time, class_type, buffer_minutes):
                current_time = self._advance_time_slot(current_time)
                continue
            
            # Create slot
            slot = self._create_slot(teacher, school, booking_date, current_time, 
                                   slot_end_time, duration_minutes)
            slots.append(slot)
    ...
    def _check_daily_booking_capacity(self, teacher: TeacherProfile, school: School, 
                                    booking_date: date, config: Dict[str, Any]) -> bool:
    ...
    def _meets_minimum_notice(self, booking_date: date, booking_time: time, 
                            minimum_notice_minutes: int) -> bool:
        """Check if slot meets minimum notice requirement."""
        now_utc = timezone.now()
        booking_datetime = timezone.make_aware(
            datetime.combine(booking_date, booking_time), pytz.UTC
        )
        minimum_allowed = now_utc + timedelta(minutes=minimum_notice_minutes)
        return booking_datetime >= minimum_allowed
    
    def _has_slot_conflicts(self, teacher: TeacherProfile, school: School, booking_date: date,
                          start_time: time, end_time: time, class_type: str = None,
        ...
        for unavailability in unavailabilities:
            ...
            if (unavailability.start_time and unavailability.end_time and
                not (end_time <= unavailability.start_time or 
                     start_time >= unavailability.end_time)):
                return True
    ...
    def _create_slot(self, teacher: TeacherProfile, school: School, booking_date: date,
                   start_time: time, end_time: time, duration_minutes: int) -> Dict[str, Any]:
