...
class GlobalSearchView(KnoxAuthenticatedAPIView):
        ...
        try:
            ...
            if school_id:
                try:
                    ...
                    return Response({"error": "Invalid school_id format"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if school_id and school_id not in user_school_ids:
                return Response(
                    {"error": "You do not have permission to search in this school"}, status=status.HTTP_403_FORBIDDEN
                )
    ...
    def _search_teachers(self, query, school_id, limit, user_school_ids):
        ...
        if school_id:
            teachers_qs = teachers_qs.filter(user__school_memberships__school_id=school_id)
    ...
    def _search_students(self, query, school_id, limit, user_school_ids):
        ...
        # Apply school filter if specified
        if school_id:
            students_qs = students_qs.filter(user__school_memberships__school_id=school_id)

        # Apply search filter
        students_qs = students_qs.filter(
            models.Q(user__name__icontains=query) | models.Q(user__email__icontains=query)
        ).distinct()[:limit]
    ...
    def _search_courses(self, query, school_id, limit):
        ...
        courses_qs = courses_qs.filter(
            models.Q(name__icontains=query) | models.Q(description__icontains=query) | models.Q(code__icontains=query)
        )[:limit]
...
class BulkTeacherActionsView(KnoxAuthenticatedAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not teacher_ids:
                return Response({"error": "Teacher IDs are required"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            logger.error(f"Error in bulk teacher actions: {e!s}")
    ...
    def _bulk_activate_teachers(self, teacher_ids, school_id):
        ...
        updated_count = SchoolMembership.objects.filter(
            user__teacher_profile__id__in=teacher_ids, school_id=school_id, role=SchoolRole.TEACHER
        ).update(is_active=True)

        return Response(
            {"message": f"Successfully activated {updated_count} teacher memberships", "updated_count": updated_count}
        )
    ...
    def _bulk_deactivate_teachers(self, teacher_ids, school_id):
        ...
        return Response(
            {"message": f"Successfully deactivated {updated_count} teacher memberships", "updated_count": updated_count}
        )
    ...
    def _bulk_invite_teachers(self, invitation_data, school_id):
        ...
        if not emails:
            return Response(
                {"error": "Email addresses are required for invitations"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def _bulk_delete_teachers(self, teacher_ids, school_id):
        ...
        return Response(
            {"message": f"Successfully removed {updated_count} teacher memberships", "removed_count": updated_count}
        )
...
class TutorDiscoveryAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.error(f"Error in tutor discovery: {e!s}")
            return Response({"error": "Failed to retrieve tutor data"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def _build_tutors_queryset(self, filters, ordering):
        ...
        if "subjects" in filters:
            ...
            queryset = queryset.filter(teacher_courses__course__name__in=filters["subjects"]).distinct()
    ...
    def _serialize_public_tutors(self, tutors_queryset, request):
        ...
        for tutor in tutors_queryset:
            ...
            for membership in tutor.user.school_memberships.filter(
                role=SchoolRole.TEACHER, is_active=True
...
class TutorOnboardingAPIView(KnoxAuthenticatedAPIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.error(f"Error getting onboarding status: {e!s}")
            return Response({"error": "Failed to get onboarding status"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not step:
                return Response({"error": "Onboarding step is required"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            logger.error(f"Error processing onboarding step: {e!s}")
            return Response(
                {"error": "Failed to process onboarding step"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class TutorOnboardingValidateStepView(TutorOnboardingAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not step:
                return Response({"error": "Step is required for validation"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            logger.error(f"Error validating onboarding step: {e!s}")
            return Response({"error": "Step validation failed"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def _validate_step_data(self, step, step_data):
        ...
        return {"valid": len(errors) == 0, "errors": errors, "step": step}
