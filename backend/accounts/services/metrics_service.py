...
from accounts.models import School, SchoolInvitation, SchoolMembership, SchoolRole
...
class SchoolMetricsService:
    ...
    def _get_student_metrics(self) -> dict[str, Any]:
        ...
        memberships = SchoolMembership.objects.filter(school=self.school, role=SchoolRole.STUDENT)
        ...
        return {"total": total, "active": active, "inactive": inactive, "trend": trends}

    def _get_teacher_metrics(self) -> dict[str, Any]:
        """Calculate teacher-related metrics"""
        memberships = SchoolMembership.objects.filter(school=self.school, role=SchoolRole.TEACHER)
    ...
    def _calculate_membership_trends(self, role: SchoolRole) -> dict[str, list[int]]:
        ...
        for i in range(7):
            ...
            count = SchoolMembership.objects.filter(school=self.school, role=role, joined_at__date=date).count()
            daily.append(count)
        daily.reverse()
        ...
        for i in range(4):
            ...
            count = SchoolMembership.objects.filter(
                school=self.school, role=role, joined_at__date__range=[start_date, end_date]
            ).count()
            weekly.append(count)
        weekly.reverse()
        ...
        for i in range(6):
            ...
            count = SchoolMembership.objects.filter(
                school=self.school, role=role, joined_at__date__range=[start_date.date(), end_date.date()]
            ).count()
    ...
    def _calculate_class_trends(self) -> dict[str, list[int]]:
        ...
        for i in range(7):
            date = (now - timedelta(days=i)).date()
            count = ClassSession.objects.filter(school=self.school, date=date, status="completed").count()
            daily.append(count)
        daily.reverse()
        ...
        for i in range(4):
            ...
            count = ClassSession.objects.filter(
                school=self.school, date__range=[start_date, end_date], status="completed"
            ).count()
            weekly.append(count)
        weekly.reverse()
...
class SchoolActivityService:
    ...
    def create_activity(cls, school, activity_type, actor, description, **kwargs):
        ...
        return SchoolActivity.objects.create(
            school=school, activity_type=activity_type, actor=actor, description=description, **kwargs
        )
