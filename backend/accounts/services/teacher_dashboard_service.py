...
from django.db.models import (
    Q, Count, Sum, Avg, Max, Min,
    Prefetch, F, Case, When, Value,
...
from accounts.models import (
    TeacherProfile, StudentProgress, ProgressAssessment,
    SchoolMembership, SchoolRole, SchoolActivity
)
...
class TeacherDashboardService:
    ...
    def _get_teacher_info(self) -> Dict[str, Any]:
        ...
        # Get courses taught
        courses_taught = []
        teacher_courses = self.teacher_profile.teacher_courses.filter(
            is_active=True
    ...
    def _get_sessions_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        base_sessions = ClassSession.objects.filter(
            teacher=self.teacher_profile
    ...
    def _get_progress_metrics(self) -> Dict[str, Any]:
        ...
        total_assessments = ProgressAssessment.objects.filter(
            student_progress__teacher=self.teacher_profile
        ).count()
        ...
        students_with_recent_assessments = ProgressAssessment.objects.filter(
            student_progress__teacher=self.teacher_profile,
            assessment_date__gte=this_month_start
        ).values('student_progress__student').distinct().count()
    ...
    def _get_recent_activities(self) -> List[Dict[str, Any]]:
        ...
        activities = SchoolActivity.objects.filter(
            Q(school_id__in=school_ids) &
            (Q(actor=self.teacher_user) | Q(target_user=self.teacher_user))
        ).select_related('actor', 'target_user', 'school').order_by('-timestamp')[:10]
    ...
    def _get_earnings_data(self) -> Dict[str, Any]:
        ...
        last_month_total = TeacherPaymentEntry.objects.filter(
            teacher=self.teacher_profile,
            session__date__gte=last_month_start,
            session__date__lte=last_month_end
        ).aggregate(total=Sum('amount_earned'))['total'] or Decimal('0.00')
        ...
        total_hours = TeacherPaymentEntry.objects.filter(
            teacher=self.teacher_profile
        ).aggregate(total_hours=Sum('hours_taught'))['total_hours'] or Decimal('0.00')
        
        # Recent payments (last 5)
        recent_payments = []
        recent_payment_entries = TeacherPaymentEntry.objects.filter(
            teacher=self.teacher_profile
    ...
    def _get_quick_stats(self) -> Dict[str, Any]:
        ...
        completion_rate = StudentProgress.objects.filter(
            teacher=self.teacher_profile
        ).aggregate(avg_completion=Avg('completion_percentage'))['avg_completion'] or Decimal('0.00')
