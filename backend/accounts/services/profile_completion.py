...
class ProfileCompletionService:
    ...
    def calculate_completion(cls, teacher_profile=None, user_id=None) -> Dict[str, Any]:
        ...
        try:
            ...
            completion_percentage = cls.calculate_weighted_score(
            ...
            is_complete = (
                completion_percentage >= cls.PROFILE_COMPLETE_THRESHOLD and
    ...
    def _assess_bio_quality(cls, bio: str) -> float:
        ...
        professional_keywords = [
            'experience', 'teach', 'education', 'student', 'year', 'qualified',
            'degree', 'master', 'bachelor', 'phd', 'certified', 'training'
        ]
    ...
    def get_profile_recommendations(cls, teacher_profile=None, user_id=None) -> List[Dict[str, str]]:
        ...
        if 'hourly_rate' in missing_critical:
            recommendations.append({
                'text': 'Set your hourly rate to help students understand your pricing.',
                'priority': 'high'
            })
        ...
        for field in priority_optional:
            if field in missing_optional:
                if field == 'availability':
                    recommendations.append({
                        'text': 'Add your availability schedule to help students book classes.',
                        'priority': 'medium'
                    })
                elif field == 'education':
                    recommendations.append({
                        'text': 'Add your educational background to build student confidence.',
                        'priority': 'medium'
                    })
                elif field == 'calendar_iframe':
                    recommendations.append({
                        'text': 'Integrate your calendar to streamline scheduling.',
                        'priority': 'medium'
                    })
        ...
        if 'phone_number' in missing_optional:
            recommendations.append({
                'text': 'Add your phone number for easier communication.',
                'priority': 'low'
            })
        
        if 'address' in missing_optional:
            recommendations.append({
                'text': 'Add your address if you offer in-person classes.',
                'priority': 'low'
            })
