...
from ..models import (
    EmailSequence, EmailSequenceStep, EmailCommunication, School,
    EmailTemplateType, EmailCommunicationType, EmailDeliveryStatus,
    TeacherInvitation, CustomUser
)
...
class EmailSequenceOrchestrationService:
    ...
    def trigger_sequence(
        ...
        teacher_invitation: Optional[TeacherInvitation] = None,
        ...
        try:
            ...
            if not sequences.exists():
                ...
                return {
                    'success': True,
                    'message': 'No sequences to trigger',
                    'triggered_sequences': 0
                }
            ...
            return {
                'success': True,
                'triggered_sequences': len(triggered_sequences),
                'sequences': triggered_sequences
            }
    ...
    def _schedule_sequence_steps(
        ...
        try:
            ...
            if not steps.exists():
                return {
                    'success': False,
                    'error': 'No active steps in sequence',
                    'scheduled_steps': 0
                }
            ...
            if not recipient_email:
                return {
                    'success': False,
                    'error': 'No recipient email in context',
                    'scheduled_steps': 0
                }
            ...
            if cls._should_prevent_duplicate_sequence(sequence, recipient_email):
                ...
                return {
                    'success': True,
                    'message': 'Sequence already active for recipient',
                    'scheduled_steps': 0
                }
    ...
    def process_due_sequence_emails(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            due_emails = EmailCommunication.objects.filter(
                delivery_status=EmailDeliveryStatus.QUEUED,
                communication_type=EmailCommunicationType.SEQUENCE,
                queued_at__lte=timezone.now()
            ).select_related(
                'template', 'school', 'sequence', 'sequence_step', 'teacher_invitation'
            ).order_by('queued_at')
            
            if not due_emails.exists():
                return {
                    'success': True,
                    'message': 'No due sequence emails to process',
                    'processed_emails': 0
                }
            ...
            for email_comm in due_emails:
                try:
                    ...
                    if result['success']:
                        ...
                        failed_count += 1
                        errors.append({
                            'email_communication_id': email_comm.id,
            ...
            logger.info(f"Processed {processed_count} sequence emails: {successful_count} successful, {failed_count} failed")
    ...
    def _evaluate_send_conditions(cls, email_comm: EmailCommunication) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'should_send': False, 'reason': f'Condition evaluation error: {str(e)}'}
    ...
    def _send_sequence_email(cls, email_comm: EmailCommunication, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            email_comm.mark_failed(f"Sequence email error: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'email_communication_id': email_comm.id
