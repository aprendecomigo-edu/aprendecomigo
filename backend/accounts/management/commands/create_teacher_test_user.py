...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--email',
            type=str,
            default='teacher.test@aprendecomigo.com',
            help='Email for the teacher test user'
        )
        parser.add_argument(
            '--name',
            type=str,
            default='Teacher Test User',
            help='Name for the teacher test user'
        )
        parser.add_argument(
            '--school-name',
            type=str,
        ...
        try:
            with transaction.atomic():
                ...
                if created:
                    ...
                    membership.save()
                    self.stdout.write(
                        self.style.SUCCESS(f'Updated membership for {user.email} to TEACHER role')
                    )
                ...
                self.stdout.write(f'Expected routing: /(teacher)/dashboard')

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error creating teacher test user: {str(e)}')
            )
            raise e