...
class TestUserSerializer(TestCase):
    ...
    def test_user_serializer_validation(self):
        ...
        # Test valid data
        valid_data = {"email": "valid@example.com", "name": "Valid User", "phone_number": "+1234567890"}
...
class TestProfileSerializers(TestCase):
    ...
    def setUp(self):
        ...
        self.teacher = TeacherProfile.objects.create(user=self.user, bio="Test Bio", specialty="Math", education="PhD")
...
class TestAuthSerializers(TestCase):
    ...
    def test_request_code_serializer_valid_email(self):
        ...
        valid_emails = ["test@example.com", "user+tag@domain.org", "test.email@subdomain.example.co.uk"]

        for email in valid_emails:
            with self.subTest(email=email):
                data = {"email": email}
                serializer = RequestCodeSerializer(data=data)
                self.assertTrue(serializer.is_valid(), f"Should accept valid email: {email}")

    def test_request_code_serializer_invalid_email(self):
        """Test RequestCodeSerializer rejects invalid email addresses."""
        invalid_emails = ["invalid-email", "@domain.com", "user@", "user..double@domain.com", ""]

        for email in invalid_emails:
            with self.subTest(email=email):
                data = {"email": email}
                serializer = RequestCodeSerializer(data=data)
                self.assertFalse(serializer.is_valid(), f"Should reject invalid email: {email}")
                self.assertIn("email", serializer.errors)

    def test_verify_code_serializer_valid_data(self):
        """Test VerifyCodeSerializer accepts valid email and code combinations."""
        valid_data = {"email": "test@example.com", "code": "123456"}
    ...
    def test_verify_code_serializer_invalid_code_length(self):
        ...
        for code in invalid_codes:
            with self.subTest(code=code):
                ...
                self.assertFalse(serializer.is_valid(), f"Should reject code with length {len(code)}")
