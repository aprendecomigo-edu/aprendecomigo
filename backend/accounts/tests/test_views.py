...
class TestUserOnboardingView(TestCase):
    ...
    def test_response_format(self):
        """Test the format of a successful response."""
        data = {
            'user': {
                'name': 'Updated User Name',
            }
        ...
        self.assertEqual(response.data['user']['name'], 'Updated User Name')
        self.assertIn('schools', response.data)
        self.assertEqual(len(response.data['schools']), 1)
        self.assertEqual(response.data['schools'][0]['name'], 'Test School')
