    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not admin_membership:
                self.stdout.write(
                    self.style.ERROR(f'No admin school found for {admin_email}')
                )
                return
            ...
            for task in tasks:
                ...
                self.stdout.write(
                    f'[{task.priority.upper()}] {task.title}{urgent_flag}{due_info}'
                )
            ...
            for schedule in upcoming_classes:
                self.stdout.write(
                    f'{schedule.scheduled_date} {schedule.start_time} - {schedule.title}'
                )
                self.stdout.write(
                    f'  Teacher: {schedule.teacher.user.name} | Student: {schedule.student.name}'
                )
            ...
            pending_tasks = Task.objects.filter(
                user=admin_user, status='pending'
            ).count()
            ...
            self.stdout.write(
                self.style.ERROR(f'User {admin_email} not found')
            )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error verifying data: {str(e)}')
            )