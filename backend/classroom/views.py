...
from .services.session_booking_service import SessionBookingError, SessionBookingService
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Message.objects.filter(channel__participants=self.request.user).order_by("-timestamp")
...
class SessionBookingViewSet(viewsets.GenericViewSet):
    ...
    def book_session(self, request):
        ...
        try:
            ...
            if not student_ids:
                return Response(
                    {"error": "student_ids is required and must not be empty"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            return Response(
                {"error": "An unexpected error occurred during booking"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def cancel_session(self, request, pk=None):
        ...
        try:
            ...
            cancellation_info = SessionBookingService.cancel_session(session_id=session_id, reason=reason)
    ...
    def adjust_duration(self, request, pk=None):
        ...
        try:
            ...
            if not actual_duration_hours:
                return Response({"error": "actual_duration_hours is required"}, status=status.HTTP_400_BAD_REQUEST)

            adjustment_info = SessionBookingService.adjust_session_duration(
                session_id=session_id, actual_duration_hours=float(actual_duration_hours), reason=reason
            )
    ...
    def get_booking_summary(self, request, pk=None):
        ...
        try:
            ...
            return Response({"success": True, "session_summary": summary})
        ...
        try:
            ...
            for student_id in student_ids:
                try:
                    ...
                    balance_info.append({"student_id": student_id, "error": "Student not found"})
            ...
            return Response(
                {
                    ...
                    "all_eligible": all(
                        info.get("has_sufficient_balance", False) for info in balance_info if "error" not in info
                    ),
                }
            )
