...
class SessionBookingService:
    ...
    def _validate_teacher_availability(teacher, date: str, start_time: str, end_time: str) -> None:
        ...
        overlapping_sessions = (
            ClassSession.objects.filter(
                teacher=teacher, date=date, status__in=[SessionStatus.SCHEDULED, SessionStatus.IN_PROGRESS]
            )
        ...
        if overlapping_sessions.exists():
            raise SessionTimingError("Teacher has conflicting session at this time")
    ...
    def cancel_session(session_id: int, reason: str = "") -> dict:
        ...
        if not session.is_trial:
            ...
            consumption_records = list(
                HourConsumption.objects.filter(class_session=session, is_refunded=False).select_related(
    ...
    def get_session_booking_summary(session_id: int) -> dict:
        ...
        try:
            session = (
                ClassSession.objects.select_related("teacher__user", "school")
        ...
        return {
            ...
            "students": [
                {"id": student.id, "name": student.name, "email": student.email} for student in session.students.all()
            ],
