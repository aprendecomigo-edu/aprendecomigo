...
const TutorSessionsPage = () => {
  const [filterStatus, setFilterStatus] = useState<'all' | 'scheduled' | 'completed' | 'cancelled'>(
    'all'
  );
  ...
  const handleSessionAction = (
    session: Session,
    action: 'start' | 'complete' | 'cancel' | 'reschedule' | 'notes'
  ) => {
    switch (action) {
      case 'start':
        Alert.alert('Iniciar Sessão', `Iniciar sessão com ${session.studentName}?`, [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Iniciar', onPress: () => console.log('Start session:', session.id) },
        ]);
        break;
      case 'complete':
        Alert.alert('Concluir Sessão', `Marcar sessão com ${session.studentName} como concluída?`, [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Concluir', onPress: () => console.log('Complete session:', session.id) },
        ]);
        break;
      case 'cancel':
        Alert.alert('Cancelar Sessão', `Cancelar sessão com ${session.studentName}?`, [
          { text: 'Não', style: 'cancel' },
          {
            text: 'Sim, Cancelar',
            style: 'destructive',
            onPress: () => console.log('Cancel session:', session.id),
          },
        ]);
        break;
      case 'reschedule':
        ...
        <VStack className="p-6" space="lg">
          ...
          <VStack space="sm">
            <HStack className="justify-between items-center">
              ...
              <Button variant="solid" onPress={() => router.push('/calendar/book')}>
            ...
            <Card variant="elevated" className="bg-white shadow-sm flex-1">
              <CardBody>
                <VStack space="xs" className="items-center">
                  ...
                  <Text className="text-lg font-bold text-gray-900">{stats.scheduled}</Text>
            ...
            <Card variant="elevated" className="bg-white shadow-sm flex-1">
              <CardBody>
                <VStack space="xs" className="items-center">
                  ...
                  <Text className="text-lg font-bold text-gray-900">{stats.completed}</Text>
            ...
            <Card variant="elevated" className="bg-white shadow-sm flex-1">
              <CardBody>
                <VStack space="xs" className="items-center">
                  ...
                  <Text className="text-lg font-bold text-gray-900">€{stats.revenue}</Text>
          ...
          <HStack space="sm" className="items-center">
            <Menu
              trigger={({ ...triggerProps }) => (
                <Pressable
                  {...triggerProps}
                  className="flex-row items-center space-x-2 p-3 bg-white border border-gray-300 rounded-lg"
                >
                ...
                <VStack space="md" className="items-center py-8">
                  ...
                  <Text className="text-sm text-gray-500 text-center max-w-sm">
                    {filterStatus === 'all'
                      ...
                      : `Nenhuma sessão ${getStatusLabel(
                          filterStatus as Session['status']
                        ).toLowerCase()} encontrada.`}
                  </Text>
                  <Button onPress={() => router.push('/calendar/book')} variant="solid">
                            ...
                            <HStack className="justify-between items-start">
                              ...
                              >
                                ...
                                {session.status === 'in_progress' && (
                                  <MenuItem
                                    onPress={() => handleSessionAction(session, 'complete')}
                                  >
                                    <Icon
                                      as={CheckCircleIcon}
                                      size="sm"
                                ...
                                <MenuItem
                                  onPress={() =>
                                    router.push(`/(tutor)/students/${session.studentId}`)
                                  }
                                >
