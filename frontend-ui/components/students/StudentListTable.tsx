    ...
    <>
      <HStack className="p-4 border-b border-gray-200 items-center">
        ...
        <Box className="w-32 md:block hidden">
          <Text className="text-sm text-gray-700">{student.educational_system?.name || 'N/A'}</Text>
        </Box>
      ...
      <Actionsheet isOpen={showActionsheet} onClose={() => setShowActionsheet(false)}>
        ...
        <ActionsheetContent>
          ...
          <ActionsheetItem 
            onPress={() => { onView(student); setShowActionsheet(false); }}
            accessibilityRole="button"
          ...
          <ActionsheetItem 
            onPress={() => { onEdit(student); setShowActionsheet(false); }}
            accessibilityRole="button"
          ...
          {student.status !== 'active' && (
            <ActionsheetItem 
              onPress={() => { onStatusChange(student, 'active'); setShowActionsheet(false); }}
              accessibilityRole="button"
          ...
          {student.status !== 'inactive' && (
            <ActionsheetItem 
              onPress={() => { onStatusChange(student, 'inactive'); setShowActionsheet(false); }}
              accessibilityRole="button"
          ...
          {student.status !== 'graduated' && (
            <ActionsheetItem 
              onPress={() => { onStatusChange(student, 'graduated'); setShowActionsheet(false); }}
              accessibilityRole="button"
          ...
          <ActionsheetItem 
            onPress={() => { onDelete(student); setShowActionsheet(false); }}
            accessibilityRole="button"
...
export const StudentListTable: React.FC<StudentListTableProps> = ({
  ...
  const handleViewStudent = React.useCallback((student: StudentProfile) => {
    router.push(`/students/${student.id}`);
  }, [router]);

  const handleEditStudent = React.useCallback((student: StudentProfile) => {
  ...
  const handleStatusChange = React.useCallback(async (student: StudentProfile, status: 'active' | 'inactive' | 'graduated') => {
    try {
  ...
  return (
    <AuthGuard requiredRoles={['school_owner', 'school_admin']}>
        ...
        <VStack space="sm">
          <HStack space="sm" className="items-center">
            ...
            <Button
              ...
              accessibilityHint={`${activeFiltersCount > 0 ? `${activeFiltersCount} filtro${activeFiltersCount > 1 ? 's' : ''} ativo${activeFiltersCount > 1 ? 's' : ''}` : 'Nenhum filtro ativo'}`}
            >
            ...
            <Box className="p-4 rounded-lg border border-gray-200 bg-gray-50">
              <VStack space="md">
                <HStack space="md" className="flex-wrap">
                  ...
                  <Box className="flex-1 min-w-48">
                    <Text className="text-sm font-medium text-gray-700 mb-1">Sistema Educacional</Text>
                    <Select
                      selectedValue={filters.educational_system?.toString() || ''}
                      onValueChange={(value) => setEducationalSystemFilter(value ? parseInt(value) : undefined)}
                    >
                      <SelectTrigger>
      ...
      {/* Table */}
      <Box className="flex-1 rounded-lg border border-gray-200" style={{ backgroundColor: COLORS.white }}>
        {/* Table Header */}
        <HStack className="p-4 border-b border-gray-200 bg-gray-50">
          <Pressable 
            ...
            accessibilityRole="button"
            accessibilityLabel={`Ordenar por nome ${sortBy === 'user__name' ? (sortDirection === 'asc' ? 'em ordem decrescente' : 'em ordem crescente') : 'em ordem crescente'}`}
          >
          ...
          <Pressable 
            ...
            accessibilityRole="button"
            accessibilityLabel={`Ordenar por ano escolar ${sortBy === 'school_year' ? (sortDirection === 'asc' ? 'em ordem decrescente' : 'em ordem crescente') : 'em ordem crescente'}`}
          >
        ...
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <Text>
              ...
              <Text className="font-semibold">{deleteStudent?.user.name}</Text>?
