    ...
    if (activeBudgetControl.monthly_limit) {
      ...
      statuses.push({
        ...
        status: percentage >= 100 ? 'exceeded' : 
                ...
                percentage >= 75 ? 'warning' : 'safe',
        daysRemaining: new Date().getDate() <= 15 ? 
        ...
        <HStack className="justify-between items-center">
          ...
          <HStack className="space-x-2">
            <Badge 
              variant="solid" 
              action={
                overallStatus === 'exceeded' || overallStatus === 'danger' ? 'error' :
                overallStatus === 'warning' ? 'warning' :
                overallStatus === 'safe' ? 'success' : 'secondary'
              }
              size="sm"
            >
              <Text className="text-xs font-medium">
                {overallStatus === 'exceeded' ? 'Over Budget' :
                 overallStatus === 'danger' ? 'High Usage' :
                 overallStatus === 'warning' ? 'Watch Usage' :
                 overallStatus === 'safe' ? 'On Track' : 'No Limits'}
              </Text>
            </Badge>
              ...
              <VStack key={budget.type} className="space-y-2">
                <HStack className="justify-between items-center">
                  <HStack className="items-center space-x-2">
                    <Icon 
                      as={budget.type === 'monthly' ? Calendar : 
                         budget.type === 'weekly' ? Calendar : Clock} 
                      size={14} 
                  ...
                  <HStack className="items-center space-x-2">
                    ...
                    <Badge
                      variant="outline"
                      action={
                        budget.status === 'exceeded' || budget.status === 'danger' ? 'error' :
                ...
                {budget.daysRemaining && !compact && (
                  <Text className="text-xs text-gray-500">
                    {budget.daysRemaining} days remaining in {budget.type === 'monthly' ? 'month' : 'week'}
                  </Text>
                )}
              </VStack>
            ))}
