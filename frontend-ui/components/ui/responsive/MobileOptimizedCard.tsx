...
import { isMobile, TouchFriendly, getResponsiveSpacing, getResponsiveTextSize } from './ResponsiveContainer';
...
export const MobileOptimizedCard: React.FC<MobileOptimizedCardProps> = ({
  ...
  const cardContent = (
    <>
      ...
      {children && (
        <CardBody className={title || subtitle ? 'pt-0' : sizeClasses.padding}>
          {children}
        </CardBody>
      )}
    </>
    ...
    <Box className={`${minTouchHeight} px-4 py-3 ${className}`}>
      <HStack space="md" className="items-center h-full">
        ...
        <VStack className="flex-1" space="xs">
          ...
          {tertiaryText && (
            <Text 
              className={`${getResponsiveTextSize('xs')} text-gray-500`}
              numberOfLines={1}
            >
              {tertiaryText}
            </Text>
      ...
      {/* Divider */}
      {divider && (
        <Box className="absolute bottom-0 left-16 right-4 h-px bg-gray-200" />
      )}
    </Box>
  );
