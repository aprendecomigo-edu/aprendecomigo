...
export const useEmailAnalytics = (autoFetch = true) => {
  ...
  const fetchAnalytics = useCallback(async (newFilters?: AnalyticsFilters) => {
    try {
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to fetch email analytics';
...
export const useEmailCommunications = (autoFetch = true) => {
  ...
  const fetchCommunications = useCallback(async (params?: {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to fetch email communications';
      setError(errorMessage);
...
export const useAnalyticsChartData = (analytics: EmailAnalytics | null) => {
  ...
  useEffect(() => {
    ...
    const deliveryChart = [
      ...
      { name: 'Pending', value: analytics.total_sent - analytics.delivered_count - analytics.failed_count, color: '#F59E0B' },
    ...
    const engagementChart = [
      ...
      { name: 'Not Opened', value: analytics.delivered_count - analytics.opened_count, color: '#6B7280' },
    ].filter(item => item.value > 0);
  ...
  useEffect(() => {
    ...
    const kpis = [
      ...
      },
      {
        ...
        color: analytics.delivery_rate >= 0.95 ? '#10B981' : analytics.delivery_rate >= 0.85 ? '#F59E0B' : '#EF4444',
      },
      {
        label: 'Open Rate',
        value: `${Math.round(analytics.open_rate * 100)}%`,
        color: analytics.open_rate >= 0.25 ? '#10B981' : analytics.open_rate >= 0.15 ? '#F59E0B' : '#EF4444',
      },
      {
        label: 'Click Rate',
        value: `${Math.round(analytics.click_rate * 100)}%`,
        color: analytics.click_rate >= 0.05 ? '#10B981' : analytics.click_rate >= 0.02 ? '#F59E0B' : '#EF4444',
      },
    ];
    ...
    if (bestTemplate) {
      insights.push(`Your "${bestTemplate.template_type}" template has the highest open rate at ${Math.round(bestTemplate.open_rate * 100)}%.`);
    }
