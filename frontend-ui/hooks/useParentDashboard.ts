...
export const useParentDashboard = () => {
  ...
  const approvePurchase = useCallback(async (requestId: string, notes?: string) => {
    try {
      ...
      setState(prev => ({
        ...
        dashboardData: prev.dashboardData ? {
          ...prev.dashboardData,
          pending_requests: prev.dashboardData.pending_requests.filter(req => req.id !== parseInt(requestId)),
          recent_approvals: [updatedRequest, ...prev.dashboardData.recent_approvals].slice(0, 10),
        } : null,
      }));
  ...
  // Derived state
  const selectedChild = state.children.find(child => child.id.toString() === state.selectedChildId) || null;
