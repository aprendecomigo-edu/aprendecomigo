...
export const useSchoolSettings = (): UseSchoolSettingsReturn => {
  ...
  const fetchSchoolSettings = useCallback(async (schoolId: number) => {
    try {
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to fetch school settings';
  ...
  const fetchEducationalSystems = useCallback(async (schoolId: number) => {
    try {
      const response = await apiClient.get(`/api/accounts/schools/${schoolId}/settings/educational-systems/`);
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to fetch educational systems';
      console.error('Error fetching educational systems:', err);
      // Don't set error state for this as it's not critical
    }
  }, []);

  const updateSchoolSettings = useCallback(async (schoolId: number, data: SchoolSettingsFormData) => {
    try {
      setSaving(true);
      setError(null);
  ...
  const uploadSchoolLogo = useCallback(async (schoolId: number, logoFile: File | any) => {
    try {
      setSaving(true);
      setError(null);
