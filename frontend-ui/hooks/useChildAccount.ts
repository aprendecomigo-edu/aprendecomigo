...
export const useChildAccount = (childId: string) => {
  ...
  const loadChildData = useCallback(async (isRefresh = false) => {
    try {
      setState(prev => ({ 
        ...prev, 
        isLoading: !isRefresh, 
  ...
  const updateChildBudget = useCallback(async (budgetData: Partial<FamilyBudgetControl>) => {
    try {
  ...
  const loadTransactionHistory = useCallback(async (page = 1) => {
    try {
      ...
      setState(prev => ({
        ...
        childData: {
          ...
          transactionHistory: page === 1 
            ...
            : [...prev.childData.transactionHistory, ...(additionalHistory.results || additionalHistory)],
        },
      }));
    } catch (error) {
  ...
  const loadPurchaseHistory = useCallback(async (page = 1) => {
    try {
      ...
      setState(prev => ({
        ...
        childData: {
          ...
          purchaseHistory: page === 1 
            ? additionalHistory.results || additionalHistory
            : [...prev.childData.purchaseHistory, ...(additionalHistory.results || additionalHistory)],
        },
      }));
    } catch (error) {
