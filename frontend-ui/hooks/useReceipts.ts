...
export function useReceipts(email?: string): UseReceiptsResult {
  ...
  const generateReceipt = useCallback(async (transactionId: string): Promise<ReceiptGenerationResponse | null> => {
    setGenerating(true);
  ...
  // Download receipt
  const downloadReceipt = useCallback(async (receiptId: string): Promise<void> => {
    setDownloading(true);
  ...
  const getPreviewUrl = useCallback(async (receiptId: string): Promise<string | null> => {
    try {
