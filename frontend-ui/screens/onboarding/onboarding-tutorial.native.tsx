...
export const OnboardingTutorial: React.FC<OnboardingTutorialProps> = ({
  ...
  // Mobile tutorial configuration
  const tutorialConfig = createMobileOnboardingTutorialConfig();
...
// Hook for managing onboarding tutorial state (native-specific)
export function useOnboardingTutorialNative() {
  const tutorialHook = useOnboardingTutorial({ userType: 'admin' });

  const startOnboardingTutorial = (userType: string = 'admin') => {
    const config = createMobileOnboardingTutorialConfig();
