...
class ClassScheduleTemplateView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context['user_is_admin'] or not hasattr(user, 'teacher_profile'):
            context['available_teachers'] = TeacherProfile.objects.filter(
                user__school_memberships__school__in=user_schools,
    ...
    def _create_schedule(self, request):
        ...
        try:
            ...
            return JsonResponse({'error': f'Failed to create schedule: {str(e)}'}, status=500)
...
class TeacherAvailabilityTemplateView(TemplateView):
    ...
    def _get_availability_by_day(self):
        ...
        for day in range(7):
            ...
            availability_by_day.append({
                'day': day,
                'availabilities': list(day_availabilities)
            })
    ...
    def _create_availability(self, request):
        ...
        try:
            ...
            school_membership = get_object_or_404(SchoolMembership,
            ...
            return JsonResponse({'error': f'Failed to create availability: {str(e)}'}, status=500)
