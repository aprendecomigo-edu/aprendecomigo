...
class SchedulerAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.edu_system, _ = EducationalSystem.objects.get_or_create(
            code="test_system", defaults={"name": "Test Educational System", "description": "Test system"}
        )

        cls.school = School.objects.create(name="Test School", description="Primary test school")

        cls.school2 = School.objects.create(name="Other School", description="Secondary test school")

        # School settings
        cls.school_settings = SchoolSettings.objects.create(
            school=cls.school, timezone="America/New_York", educational_system=cls.edu_system
        )
        ...
        cls.teacher_user2 = CustomUser.objects.create_user(
            email="teacher2@test.com", name="Second Teacher", password="testpass123"
        )
        ...
        cls.student_user2 = CustomUser.objects.create_user(
            email="student2@test.com", name="Second Student", password="testpass123"
        )

        cls.unauthorized_user = CustomUser.objects.create_user(
            email="unauthorized@test.com", name="Unauthorized User", password="testpass123"
        )

        # Teacher profiles
        cls.teacher_profile = TeacherProfile.objects.create(user=cls.teacher_user, bio="Primary test teacher")

        cls.teacher_profile2 = TeacherProfile.objects.create(user=cls.teacher_user2, bio="Secondary test teacher")

        # School memberships
        SchoolMembership.objects.create(
            user=cls.admin_user, school=cls.school, role=SchoolRole.SCHOOL_OWNER, is_active=True
        )

        SchoolMembership.objects.create(
            user=cls.teacher_user, school=cls.school, role=SchoolRole.TEACHER, is_active=True
        )

        SchoolMembership.objects.create(
            user=cls.teacher_user2, school=cls.school, role=SchoolRole.TEACHER, is_active=True
        )

        SchoolMembership.objects.create(
            user=cls.student_user, school=cls.school, role=SchoolRole.STUDENT, is_active=True
        )

        SchoolMembership.objects.create(
            user=cls.student_user2, school=cls.school, role=SchoolRole.STUDENT, is_active=True
        )
    ...
    def assertCreatedWithData(self, response, expected_data=None):
        ...
        if expected_data:
            for key, value in expected_data.items():
                self.assertEqual(response.data.get(key), value, f"Expected {key}={value}, got {response.data.get(key)}")
