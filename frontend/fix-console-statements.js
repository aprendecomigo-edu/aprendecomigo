...
class ConsoleStatementFixer {
  ...
  processFile(filePath) {
    try {
      ...
      const consoleRegex = /^(\s*)(console\.(log|info|debug|warn|error)\s*\([^)]*\);?)/gm;
      
      content = content.replace(consoleRegex, (match, indent, consoleStatement, method) => {
        fileChanges++;
  ...
  async processBatch(files, batchIndex) {
    if (__DEV__) {
      console.log(`\n📦 Processing batch ${batchIndex + 1}/${Math.ceil(files.length / BATCH_SIZE)
    }} (${files.length} files)`);
  ...
  async run() {
    if (__DEV__) {
      console.log('🚀 Starting console statement fixing process...\n');
    }
  ...
  groupFilesByPriority(files) {
    ...
    files.forEach(file => {
      ...
      } else if (file.includes('/utils/') || file.includes('/services/') || file.includes('/lib/')) {
...
// Run the fixer
const fixer = new ConsoleStatementFixer();
