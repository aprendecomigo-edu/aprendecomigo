...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({
  ...
  const { getStepName, getMilestoneMessage, getProgressMessage, getNextStepHint } =
  ...
  const steps = useMemo(
    () => [
      ...
      { number: 3, name: 'Grade Levels', description: 'Choose the grade levels you work with' },
      {
        number: 4,
        name: 'Availability',
        description: 'Set your teaching schedule and preferences',
      },
      {
        number: 5,
        name: 'Rates & Compensation',
        description: 'Define your hourly rates and payment terms',
      },
      { number: 6, name: 'Credentials', description: 'Add your education and teaching experience' },
      {
        number: 7,
        name: 'Profile Marketing',
        description: 'Create your bio and teaching philosophy',
      },
        ...
        <VStack space="sm">
          ...
          {progress && (
            <Text className="text-xs text-gray-500">
              ...
              {getStepName(progress.current_step)}
            </Text>
          )}
        </VStack>
      </Card>
    ...
    <VStack space="lg">
      ...
      <Card className="p-6">
        <VStack space="md">
          ...
          {progress && (
            <Text className="text-gray-600 text-center">{getProgressMessage(progress)}</Text>
          )}
        </VStack>
      </Card>
                    ...
                    <Box className="flex-shrink-0">
                      {status === 'completed' ? (
                        ...
                        <Box className="w-6 h-6 border-2 border-blue-600 rounded-full bg-blue-100 items-center justify-center">
                          <Text className="text-xs font-bold text-blue-600">{step.number}</Text>
                        </Box>
                      ) : (
