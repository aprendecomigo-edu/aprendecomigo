    ...
    <Card className={`p-4 ${style.containerClass} ${notification.is_read ? 'opacity-75' : ''}`}>
      <VStack space="sm">
          ...
          <HStack space="sm" className="items-start flex-1">
            ...
            <VStack space="xs" className="flex-1">
              <HStack space="xs" className="items-center flex-wrap">
                ...
                {!notification.is_read && <NotificationDot type={style.badgeAction} size="sm" />}
              </HStack>
        ...
        <HStack className="items-center justify-between">
          <Text className="text-xs text-typography-500">{formatDate(notification.created_at)}</Text>

          <HStack space="sm" className="items-center">
            <Badge variant="outline" action={style.badgeAction} size="xs">
              <Text className="text-xs capitalize">{notification.priority}</Text>
            </Badge>

            <Badge variant="outline" action="secondary" size="xs">
              <Text className="text-xs">{notification.notification_type.replace('_', ' ')}</Text>
            </Badge>
          </HStack>
        </HStack>
      </VStack>
        ...
        <HStack className="items-center justify-between">
          <VStack space="xs">
            <HStack className="items-center">
              ...
              {state.unreadCount > 0 && (
                <NotificationBadge count={state.unreadCount} type="error" size="sm" />
              )}
            </HStack>
        ...
        {showFilters && (
          <HStack space="sm" className="flex-wrap">
            ...
            <Select
              ...
              onValueChange={value =>
                setFilters(prev => ({ ...prev, status: value as 'all' | 'unread' | 'read' }))
              }
            >
            ...
            {state.unreadCount > 0 && (
              <Button action="primary" variant="outline" size="sm" onPress={actions.markAllAsRead}>
                <ButtonIcon as={CheckCircle} />
          ...
          {filteredNotifications.length === 0 ? (
            <VStack space="sm" className="items-center py-8">
              ...
              <VStack space="xs" className="items-center">
                <Text className="text-typography-600 font-medium">No notifications</Text>
