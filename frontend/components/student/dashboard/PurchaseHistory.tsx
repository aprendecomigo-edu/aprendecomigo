 ...
 */
function ConsumptionItem({ consumption }: { consumption: ConsumptionRecord }) {
  return (
    <HStack className="items-center justify-between p-3 bg-background-50 rounded-md">
      <HStack space="sm" className="items-center flex-1">
        ...
        <VStack space="0" className="flex-1">
          ...
          <Text className="text-xs text-typography-600">{consumption.description}</Text>
          {consumption.teacher_name && (
            <Text className="text-xs text-typography-500">with {consumption.teacher_name}</Text>
          )}
        </VStack>
      </HStack>
 ...
 */
function PurchaseItem({ purchase }: { purchase: PurchaseHistoryItem }) {
  ...
  const statusConfig =
    PAYMENT_STATUS_CONFIG[purchase.payment_status] || PAYMENT_STATUS_CONFIG.pending;
        ...
        <VStack space="sm">
          ...
          <HStack className="items-center justify-between">
            <HStack space="sm" className="items-center flex-1">
              ...
              <VStack space="0" className="flex-1">
                <Text className="font-semibold text-typography-900">{purchase.plan_name}</Text>
                <Text className="text-xs text-typography-600">
                  {purchase.plan_type === 'package' ? 'Package' : 'Subscription'} • ID:{' '}
            ...
            <VStack space="xs" className="items-end">
              ...
              <Badge variant="outline" action={getStatusColor()} size="sm">
          ...
          <VStack space="xs">
            <HStack className="items-center justify-between">
              <Text className="text-sm text-typography-700">Usage Progress</Text>
              <Text className="text-sm text-typography-600">{usagePercentage.toFixed(0)}%</Text>
            </HStack>
          ...
          <VStack space="xs">
            <HStack className="items-center justify-between">
              <Text className="text-sm text-typography-700">Hours Included:</Text>
            ...
            <HStack className="items-center justify-between">
              <Text className="text-sm text-typography-700">Hours Used:</Text>
              <Text className="text-sm font-semibold text-error-600">
          ...
          {showConsumption && purchase.consumption_history && (
            <VStack space="xs">
              {purchase.consumption_history.map(consumption => (
                <ConsumptionItem key={consumption.id} consumption={consumption} />
              ))}
            </VStack>
          )}
          ...
          <VStack space="sm">
            <HStack className="items-center justify-between">
              ...
              {purchase.consumption_history && purchase.consumption_history.length === 0 && (
                <Text className="text-xs text-typography-500">No usage yet</Text>
              )}
            </HStack>
  ...
  return (
    <VStack space="md">
      <HStack className="items-center justify-between">
        ...
        <Pressable onPress={() => setShowFilters(!showFilters)} className="flex-row items-center">
      ...
      {showFilters && (
        <VStack space="md">
          {/* Active Only Toggle */}
          <HStack className="items-center justify-between">
            <Text className="text-sm font-medium text-typography-800">Show Active Only</Text>
            <Switch
              size="sm"
          ...
          <HStack className="items-center justify-between">
            <Text className="text-sm font-medium text-typography-800">Include Usage History</Text>
            <Switch
              size="sm"
    ...
    <VStack space="lg">
      ...
      <HStack className="items-center justify-between">
        <VStack space="xs">
          ...
          {purchases && (
            <Text className="text-sm text-typography-600">{purchases.count} total purchases</Text>
          )}
        </VStack>
      ...
      {/* Filters */}
      <Card className="p-4">
        <PurchaseFilters filters={filters} onFiltersChange={onFiltersChange} />
      </Card>
      ...
      {purchases && purchases.results.length > 0 ? (
        <VStack space="md">
          {purchases.results.map(purchase => (
            <PurchaseItem key={`${purchase.id}-${purchase.transaction_id}`} purchase={purchase} />
          ))}
        ...
        <Card className="p-8">
          <VStack space="md" className="items-center">
            ...
            <VStack space="xs" className="items-center">
              <Text className="font-medium text-typography-600">No Purchases Found</Text>
