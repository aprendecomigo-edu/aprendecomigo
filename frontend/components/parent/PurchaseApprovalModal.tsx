          ...
          <HStack className="justify-between items-center w-full">
            <HStack className="space-x-3">
              ...
              <VStack>
                ...
                <Text className="text-sm text-gray-600">{approval.pricing_plan.name}</Text>
              </VStack>
            </HStack>
        ...
        <ModalBody>
          <ScrollView>
            <VStack className="space-y-6">
                ...
                <VStack className="space-y-3">
                  ...
                  {budgetImpacts.map(impact => (
                    <Card
                      key={impact.type}
                      className={`border ${
                        impact.exceeds ? 'border-red-200 bg-red-50' : 'border-gray-200'
                      }`}
                    >
                      <CardContent className="p-3">
                        <VStack className="space-y-2">
                          <HStack className="justify-between items-center">
                            ...
                            <Text
                              className={`text-sm font-semibold ${
                                impact.exceeds ? 'text-red-600' : 'text-gray-900'
                              }`}
                            >
                          ...
                          <Progress value={Math.min(impact.percentage, 100)} className="h-2" />
                          ...
                          <HStack className="justify-between items-center">
                            ...
                            <Text
                              className={`text-xs font-medium ${
                                impact.exceeds ? 'text-red-600' : 'text-gray-600'
                              }`}
                            >
                              {Math.round(impact.percentage)}%{impact.exceeds && ' (Over Budget)'}
                            </Text>
                          </HStack>
                        </VStack>
                      </CardContent>
              ...
              {childSpendingHistory?.last_purchases &&
                childSpendingHistory.last_purchases.length > 0 && (
                  <VStack className="space-y-3">
                    <HStack className="items-center space-x-2">
                    ...
                    <VStack className="space-y-2">
                      {childSpendingHistory.last_purchases.slice(0, 3).map((purchase, index) => (
                        <HStack
                          key={index}
                          className="justify-between items-center p-3 bg-gray-50 rounded-lg"
                        >
                          <VStack>
                            <Text className="text-sm font-medium text-gray-900">
                              {purchase.plan_name}
                  ...
                  <VStack className="space-y-2">
                    ...
                    <VStack className="space-y-1">
                      {getSuggestedResponses(selectedAction).map((suggestion, index) => (
                        ...
                        >
                          <Text className="text-sm text-blue-800">"{suggestion}"</Text>
                        </Pressable>
                      ))}
                    </VStack>
                  </VStack>
