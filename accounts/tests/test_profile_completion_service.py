...
class ProfileCompletionServiceTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="teacher@test.com", name="Test Teacher", phone_number="+351123456789"
        )

        self.school = School.objects.create(name="Test School", description="A test school")

        SchoolMembership.objects.create(user=self.user, school=self.school, role=SchoolRole.TEACHER)

        # Get or create educational system
        self.edu_system, created = EducationalSystem.objects.get_or_create(code="pt", defaults={"name": "Portugal"})

        # Create some courses
        self.course1 = Course.objects.create(
            name="Mathematics", code="MAT", educational_system=self.edu_system, education_level="ensino_basico_1_ciclo"
        )

        self.course2 = Course.objects.create(
            name="Physics", code="PHY", educational_system=self.edu_system, education_level="ensino_secundario"
        )
    ...
    def test_calculate_completion_basic_profile(self):
        ...
        # Add one course
        TeacherCourse.objects.create(teacher=teacher_profile, course=self.course1, hourly_rate=Decimal("25.00"))
    ...
    def test_calculate_completion_complete_profile(self):
        ...
        TeacherCourse.objects.create(teacher=teacher_profile, course=self.course1, hourly_rate=Decimal("35.00"))
        TeacherCourse.objects.create(teacher=teacher_profile, course=self.course2, hourly_rate=Decimal("30.00"))
    ...
    def test_identify_missing_fields(self):
        ...
        teacher_profile = TeacherProfile.objects.create(user=self.user, bio="Basic bio")
    ...
    def test_calculate_weighted_score(self):
        ...
        total_score = ProfileCompletionService.calculate_weighted_score(basic_score, teaching_score, professional_score)
    ...
    def test_profile_quality_scoring(self):
        ...
        # Add courses
        TeacherCourse.objects.create(teacher=teacher_profile, course=self.course1, hourly_rate=Decimal("40.00"))
    ...
    def test_bulk_completion_calculation(self):
        ...
        profiles = []
        for i in range(3):
            user = User.objects.create_user(email=f"teacher{i}@test.com", name=f"Teacher {i}")
            SchoolMembership.objects.create(user=user, school=self.school, role=SchoolRole.TEACHER)
            profile = TeacherProfile.objects.create(
                user=user, bio=f"Bio for teacher {i}", hourly_rate=Decimal("25.00") if i > 0 else None
            )
    ...
    def test_school_completion_analytics(self):
        ...
        for i in range(5):
            user = User.objects.create_user(email=f"teacher{i}@school.com", name=f"Teacher {i}")
            SchoolMembership.objects.create(user=user, school=self.school, role=SchoolRole.TEACHER)
            TeacherProfile.objects.create(
                user=user, bio=f"Bio {i}" if i < 3 else "", hourly_rate=Decimal("25.00") if i < 2 else None
            )
