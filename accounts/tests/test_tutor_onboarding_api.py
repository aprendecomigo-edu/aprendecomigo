...
from accounts.models import Course, EducationalSystem, TeacherCourse, TeacherProfile
...
class TutorOnboardingAPITestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create test user
        self.user = User.objects.create_user(email="test@tutor.com", first_name="Test", last_name="Tutor")
        ...
        self.educational_system = EducationalSystem.objects.create(
            name="Portugal", code="PT", description="Portuguese Educational System"
        )
        ...
        data = {
            ...
            "data": {
                "course_selection": {
                    ...
                    "selected_courses": [
                        {"course_id": self.course1.id, "hourly_rate": 25.00, "expertise_level": "intermediate"}
                    ],
                }
            },
        }
...
class TutorOnboardingIntegrationTest(TutorOnboardingAPITestCase):
    ...
    def test_teacher_profile_model_integration(self):
        ...
        teacher_profile = TeacherProfile.objects.create(user=self.user, bio="Test teacher bio", hourly_rate=30.00)

        # Verify profile was created
        self.assertTrue(TeacherProfile.objects.filter(user=self.user).exists())

        # Create teacher course relationship
        teacher_course = TeacherCourse.objects.create(teacher=teacher_profile, course=self.course1, hourly_rate=25.00)

        # Verify relationship was created
        self.assertTrue(TeacherCourse.objects.filter(teacher=teacher_profile, course=self.course1).exists())
