...
class UserSchoolIntegrityTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.existing_school = School.objects.create(
            ...
            description="A pre-existing school"
        )
...
class UserSchoolIntegrityManagementCommandTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.orphaned_user1 = User.objects.create_user(
            email="orphaned1@example.com",
            name="Orphaned User 1"
        )
        self.orphaned_user2 = User.objects.create_user(
            email="orphaned2@example.com", 
            name="Orphaned User 2"
        )
        ...
        self.proper_user = User.objects.create_user(
...
class UserSchoolIntegrityTransactionTestCase(BaseTransactionTestCase):
    ...
    def test_atomic_user_school_creation(self):
        ...
        with patch('accounts.models.schools.School.objects.create') as mock_create:
            ...
            self.assertFalse(
                SchoolMembership.objects.filter(user=user).exists()
            )
    ...
    def test_user_with_special_characters_in_name(self):
        ...
        for name in special_names:
            with self.subTest(name=name):
                user = User.objects.create_user(
                    email=f"special_{len(name)}@example.com",
                    name=name
                )
