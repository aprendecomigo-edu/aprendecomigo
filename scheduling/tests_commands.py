...
class SyncCalendarCommandTest(TestCase):
    ...
    @patch('scheduling.management.commands.sync_calendar.DEFAULT_CALENDAR_ID', TEST_CALENDAR_ID)
    def test_sync_calendar_command(self, mock_social_account, mock_sync_events):
        ...
        self.assertIn("Calendar sync completed: 10 created, 5 updated, 15 total events processed", output)
    ...
    def test_sync_calendar_with_date_range(self, mock_social_account, mock_sync_events):
        ...
        self.assertIn("Calendar sync completed: 5 created, 0 updated, 5 total events processed", output)
    ...
    @patch('scheduling.management.commands.sync_calendar.SocialAccount')
    @patch('scheduling.management.commands.sync_calendar.DEFAULT_CALENDAR_ID', TEST_CALENDAR_ID)    
    def test_sync_calendar_with_placeholder_warnings(self, mock_social_account, mock_sync_events):
        """Test command with placeholder warnings enabled"""
        # Setup mocks
        mock_social_account.objects.get.return_value = self.social_account
        
        # Make sync_events create a side effect that prints placeholder messages
        def side_effect(*args, **kwargs):
            print("Created placeholder teacher: teacher@placeholder.com (needs profile completion)")
            print("Created placeholder student: student@placeholder.com (needs profile completion)")
            return (2, 0, 2)
