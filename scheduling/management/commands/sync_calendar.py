...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=30,
            help='Number of days to fetch events for (ignored if both start-date and end-date are provided)'
        )
        parser.add_argument(
            '--warn-placeholder',
            action='store_true',
        ...
        parser.add_argument(
            '--start-date',
            help='Start date for fetching events (format: YYYY-MM-DD)'
        )
        parser.add_argument(
            '--end-date',
            help='End date for fetching events (format: YYYY-MM-DD)'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Found Google account for admin: {admin_email}"
            ))
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"Calendar sync completed: {created} created, {updated} updated, {total} total events processed"
            ))
