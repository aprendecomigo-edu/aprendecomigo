...
from .models import ClassType, ClassSession, Subject
...
def manage_class_types(request):
    ...
    if request.method == 'POST':
        ...
        if not name or not hourly_rate:
            messages.error(request, "Both Class Type Code and Hourly Rate are required.")
    ...
    return render(request, 'scheduling/manage_class_types.html', {'class_types': class_types})
...
def edit_class_type(request, class_type_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if ClassType.objects.filter(name=name).exclude(id=class_type_id).exists():
                messages.error(request, f"Another class type with code '{name}' already exists.")
                return redirect('edit_class_type', class_type_id=class_type_id)
    ...
    return render(request, 'scheduling/edit_class_type.html', {'class_type': class_type})
...
def delete_class_type(request, class_type_id):
    ...
    if class_type.sessions.exists():
        messages.error(request, f"Cannot delete class type '{class_type.name}' because it is used by {class_type.sessions.count()} sessions.")
        return redirect('manage_class_types')
    
    try:
        class_type_name = class_type.name
        class_type.delete()
        messages.success(request, f"Class Type '{class_type_name}' deleted successfully.")
    except Exception as e:
...
def view_sessions(request):
    ...
    sessions = ClassSession.objects.select_related('teacher', 'class_type', 'subject').all().order_by('-start_time')
    ...
    if date_range == 'today':
        ...
        sessions = sessions.filter(start_time__year=today.year, start_time__month=today.month)
...
def edit_session(request, session_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            students_str = ", ".join([student.name for student in session.students.all()])
            title = students_str
            if not attended:
                title += " - FALTOU"
            session.title = title
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if start_date and end_date:
                ...
                messages.success(request, f"Successfully synced {total} events ({created} created, {updated} updated) from {start_date} to {end_date}.")
            else:
                # Use days
                ...
                messages.success(request, f"Successfully synced {total} events ({created} created, {updated} updated) for the next {days} days.")
...
def filter_sessions(request):
    ...
    return render(request, 'partials/sessions_table.html', {
        'sessions': page_obj,
        'is_paginated': page_obj.has_other_pages(),
        'page_obj': page_obj
    })
...
def upcoming_classes(request):
    ...
    ).order_by('start_time')[:5]  # Limit to 5 upcoming classes
    
    return render(request, 'partials/upcoming_classes.html', {
...
def today_schedule(request):
    ...
    return render(request, 'partials/teacher_schedule.html', {
        'schedule': schedule
    })
